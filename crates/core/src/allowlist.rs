...
mod tests {
    ...
    /// kimi-1 dogfood #129: bare path globs (no `path:` prefix) used
    /// to emit a warning and silently drop the entry. Now they're
    /// accepted as gitignore-style shorthand.
    #[test]
    fn bare_globs_parse_as_path_entries_gitignore_style() {
        let content = "*.log\nnode_modules/\nvendor/**/*.json\n";
        let al = Allowlist::parse(content);
        assert_eq!(
            al.ignored_paths.len(),
            3,
            "all three bare globs must register as path entries; got {:?}",
            al.ignored_paths
        );
    ...
    #[test]
    fn bare_sha256_hex_parses_as_hash_entry_gitignore_style() {
        let hex = "9d6060e21ef8d5daec9cfe4a44b1b1bc9792246bfad28210edaaa1782a8a676a";
        let al = Allowlist::parse(hex);
        assert_eq!(al.credential_hashes.len(), 1);
        assert!(al.is_raw_hash_ignored(hex));
    }

    /// Mixed file: explicit prefixed entries + bare globs in the same
    /// `.keyhogignore` should all load.
    #[test]
    fn mixed_prefixed_and_bare_entries_all_load() {
        let content = "\
# generated by `keyhog scan ... | tee -a .keyhogignore`
detector:asana-pat
...
";
        ...
        assert!(
            al.ignored_paths.iter().any(|p| p == "*.log"),
            "bare glob registered without prefix; got {:?}",
            al.ignored_paths
        );
