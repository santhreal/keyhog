...
#[test] fn prop_debug_redacts(cred in "[A-Za-z0-9]{8,40}") {
        let dbg = format!("{:?}", Credential::from(cred.as_str()));
...
#[test] fn prop_display_redacts(cred in "[^\0]{1,30}") { let d=format!("{}", Credential::from(cred.as_str())); prop_assert!(d.contains("redacted")); }
}
