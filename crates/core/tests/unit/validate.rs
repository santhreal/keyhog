...
fn accepts_unbounded_simple_class_nested_in_counted_group() {
    ...
    // detector-authored inter-keyword separator `[_\-\s]*` inside a
    // `(?:…){1,3}` anchor (deepnote-api-credentials); before the fix it scored
    // a fictitious 3 x 1000 = 3000 and rejected the whole corpus. A genuinely
    // COUNTED explosion (`(?:X{500}){3}`) is still rejected (test above).
