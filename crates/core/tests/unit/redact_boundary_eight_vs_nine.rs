...
//! Credentials >= 9 chars must preserve first 4 and last 4, separated by "...".
...
fn redact_exactly_nine_ascii_chars_reveals_edges_not_middle() {
    // Boundary: 9 chars (exclusive of the 8-char rule) must reveal first 4 + last 4.
    let result = redact("abcdefghi");
    assert_eq!(result, "abcd...ghi");
    // Explicit assertion: result must be exactly this length and format.
    assert_eq!(result.len(), 10, "format is 'first4...last3' (9 total chars -> 3 overlap)");
}
...
fn redact_nine_utf8_graphemes_reveals_edges() {
    // UTF-8 boundary: 9 graphemes → first 4 + "..." + last 4 (1 char overlap)
    let result = redact("αβγδεζηθι");
    assert!(result.starts_with("αβγδ"), "first 4 graphemes must be preserved");
    assert!(result.ends_with("ζηθι"), "last 4 graphemes must be preserved");
    assert!(result.contains("..."), "ellipsis must separate first and last");
}
...
fn redact_cjk_multibyte_nine_graphemes_preserves_edges() {
    ...
    assert!(result.starts_with("一二三四"));
    assert!(result.ends_with("六七八九"));
