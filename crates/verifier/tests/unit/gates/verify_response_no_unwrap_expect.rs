//! Gate `verify::response`: no .unwrap( / .expect( in production source lines.

#[test]
fn verify_response_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "verify::response: unwrap/expect in production source at {:?}",
