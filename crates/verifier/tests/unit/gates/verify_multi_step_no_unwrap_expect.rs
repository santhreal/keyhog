//! Gate `verify::multi_step`: no .unwrap( / .expect( in production source lines.

#[test]
fn verify_multi_step_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "verify::multi_step: unwrap/expect in production source at {:?}",
