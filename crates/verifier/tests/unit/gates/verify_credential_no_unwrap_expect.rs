//! Gate `verify::credential`: no .unwrap( / .expect( in production source lines.

#[test]
fn verify_credential_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "verify::credential: unwrap/expect in production source at {:?}",
