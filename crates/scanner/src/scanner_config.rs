...
pub struct ScannerConfig {
    ...
    pub profile: bool,
    /// Emit low-level phase timing traces for GPU/perf investigation. Implies
    /// [`ScannerConfig::profile`]: a trace is the deeper level of the same
...
impl ScannerConfig {
    /// Collapse the two request flags onto the single process-wide measurement
    /// level the profiler owns.
    ///
    /// `perf_trace` is the deeper level of the same measurement, so it implies
    /// stage recording. This is the only place the scanner translates a request
    /// into a measurement decision.
    pub fn profile_detail(&self) -> keyhog_profile::Detail {
        if self.perf_trace {
            keyhog_profile::Detail::Diagnostic
        } else if self.profile {
            keyhog_profile::Detail::Stages
        } else {
            keyhog_profile::Detail::Off
        }
    }
}
...
// The configuration cases live in `tests/unit/scanner_config_cases.rs`. They
// were 165 lines against 548 of configuration plumbing. The `#[path]`
// include keeps them compiled with the crate so they still reach the
// crate-private builder state.
#[cfg(test)]
#[path = "../tests/unit/scanner_config_cases.rs"]
mod config_tests;
