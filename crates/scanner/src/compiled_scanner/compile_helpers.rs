...
#[cfg(feature = "simdsieve")]
pub(super) struct StreamingHotPatternSlots {
    seen: std::collections::HashSet<String>,
    slots: Vec<crate::simdsieve_prefilter::HotPatternSlot>,
}

#[cfg(feature = "simdsieve")]
impl StreamingHotPatternSlots {
    pub(super) fn new() -> Self {
        Self {
            seen: std::collections::HashSet::with_capacity(16),
    ...
    pub(super) fn push(
        ...
        if self
            .slots
            .len()
            .saturating_add(detector.simdsieve_prefixes.len())
            > 16
        {
            return Err(crate::error::ScanError::Config(
                "hydrated detector plan declares more than 16 simdsieve prefixes".to_owned(),
            ));
        }
        if detector.simdsieve_prefixes.is_empty() {
            return Ok(());
        ...
        for prefix in &detector.simdsieve_prefixes {
            ...
            self.slots.push(HotPatternSlot {
                prefix: prefix.as_bytes().into(),
                validator: validator.clone(),
                ac_map_index,
            });
    ...
    pub(super) fn finish(self) -> Vec<crate::simdsieve_prefilter::HotPatternSlot> {
        self.slots
    }
}
