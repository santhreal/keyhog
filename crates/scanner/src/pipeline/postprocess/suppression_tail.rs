...
pub(super) fn check_shape_context_and_b64_gates(
    ...
    // ── 4. Known fake sequences (dominant substring; gated for named detectors) ──
    if !bypass_shape_gates {
    ...
    // Gated by `bypass_shape_gates` — monotonic-hex heuristics suppress Algolia-style keys.
    if !bypass_shape_gates && crate::context::is_known_example_credential(credential) {
    ...
    if let Some(path) = path {
        ...
        let is_example_path = crate::platform_compat::path_has_component(path, "example")
            || crate::platform_compat::path_has_component(path, "examples")
            || crate::platform_compat::path_has_component(path, "test")
            || crate::platform_compat::path_has_component(path, "tests")
            || crate::platform_compat::path_has_component(path, "fixture")
            || crate::platform_compat::path_has_component(path, "fixtures");
    ...
    if !skip_b64_decode_recheck {
        if let Some(decoded) = try_decode_b64_to_utf8(credential) {
            ...
            if !decoded.is_empty()
                ...
                && super::suppression_inner::should_suppress_inner(
                    &decoded,
                    path,
                    context,
                    source_type,
