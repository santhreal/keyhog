            ...
            let decode_parent = |chunk: &Chunk,
             ...
             -> crate::error::Result<()> {
                ...
                if !decoded_chunks.is_empty() {
                    ...
                    .map_err(|e| crate::ScanError::Config(e.to_string()))?;
                    let mut seen: HashSet<_> = raw_findings.iter().map(key).collect();
                    *matches = raw_findings;
                    matches.extend(resolved.into_iter().filter(|m| seen.insert(key(m))));
                }
                Ok(())
            };
