...
//! `fallback.rs`. Pure `&str` functions — regex prefix-anchorability and
//! focus-window truncation — with no engine-state coupling.
//! `truncate_for_prefilter` is re-exported at the crate root (see lib.rs);
//! `regex_prefix_anchorable` and `truncate_src` are pub(crate) for fallback.rs's
...
pub(crate) fn focus_floor_boundary(s: &str, mut idx: usize) -> usize {
    if idx >= s.len() {
        return s.len();
    }
...
pub(crate) fn focus_ceil_boundary(s: &str, mut idx: usize) -> usize {
    if idx >= s.len() {
        return s.len();
    }
