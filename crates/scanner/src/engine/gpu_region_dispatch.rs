...
impl CompiledScanner {
    ...
    ) -> std::result::Result<super::CoalescedScanOutcome, super::gpu_forced::SelectedGpuDispatchError>
    {
        ...
        let mut dispatch_presence = |haystack: &[u8],
            ...
            let expected_presence_words = rows.checked_mul(presence_words).ok_or_else(|| {
                ...
                crate::gpu::scan_gpu_literal_evidence_by_region_resident(
                    ...
                    |presence, literal_matches| {
                        ...
                        for literal_match in literal_matches {
                            ...
                            let row_idx = logical_start.checked_add(region).ok_or_else(|| {
            ...
            match result {
                ...
                Err(error) if recover_dispatch_faults => {
                    ...
                    for region in 0..rows {
                        ...
                        let chunk_index = logical_start.checked_add(region).ok_or_else(|| {
                            "GPU recovery logical chunk index overflows host usize".to_string()
                        })?;
                        let byte_start = logical_byte_base;
                        let byte_end = byte_start.checked_add(source.len()).ok_or_else(|| {
            ...
            let (summary, next_cursor) = if oversized {
                ...
                let summary = for_each_region_presence_window(
                    ...
                    |haystack, range| {
                        ...
                        dispatch_presence(haystack, &[0], logical_row, 1, range.start, &mut reduce)
                    },
                );
                if summary.is_ok() {
                    let t_derive = kh.then(std::time::Instant::now);
