//! Anchored-position verification for the shared-anchor fallback localizer.
//!
//! Split out of `fallback_anchor.rs` (Law 5, 500-LOC ceiling). That file owns
//! the `FallbackAnchorIndex` build + candidate-collection machinery; this is the
...
//! emitting byte-identical matches via `process_match`. See `fallback_anchor.rs`
