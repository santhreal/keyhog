...
pub const MATCHER_ARTIFACT_MAX_ENTRIES: usize = keyhog_core::CacheKind::MatcherArtifacts
    .default_policy()
    .max_entries;
static CONFIGURED_CACHE_DIR: LazyLock<parking_lot::RwLock<Option<PathBuf>>> =
    LazyLock::new(|| parking_lot::RwLock::new(None));
static CONFIGURED_CACHE_DISABLE_REASON: LazyLock<
    parking_lot::RwLock<MatcherArtifactCacheDisableReason>,
> = LazyLock::new(|| parking_lot::RwLock::new(MatcherArtifactCacheDisableReason::ConfiguredOff));
...
pub fn set_matcher_artifact_cache_disable_reason(reason: MatcherArtifactCacheDisableReason) {
    *CONFIGURED_CACHE_DISABLE_REASON.write() = reason;
}
...
pub fn configured_matcher_artifact_cache_disable_reason() -> MatcherArtifactCacheDisableReason {
    *CONFIGURED_CACHE_DISABLE_REASON.read()
}
