...
// The predicate boundary suite lives in
// `tests/unit/context_placeholder_predicates.rs`. It was 167 lines against
// 263 of implementation. The `#[path]` include keeps it compiled with the
// crate: it pins the private `is_empty_input_hash` and
// `is_hex_sequential_placeholder` near-miss boundaries, which have no
// public surface.
#[cfg(test)]
#[path = "../../tests/unit/context_placeholder_predicates.rs"]
mod sequential_placeholder_tests;

// The empty-input-hash suite lives in
// `tests/unit/context_placeholder_empty_hash.rs`, kept in-crate by the
// `#[path]` include because `is_empty_input_hash` is private.
#[cfg(test)]
#[path = "../../tests/unit/context_placeholder_empty_hash.rs"]
