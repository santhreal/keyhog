...
//! `fallback.rs`. Each toggle has a compiled shipped DEFAULT plus a PER-SCANNER
...
//! not a detection set) — see the per-method docs. The toggles span the fallback
...
//! Re-exported through `engine::fallback` (`pub use crate::tuning::*`).
...
pub(crate) struct ScannerTuning {
    ...
    /// Override for fallback plain-pattern localization.
    fallback_localizer: AtomicU8,
...
impl ScannerTuning {
    ...
    pub(crate) const fn from_defaults() -> Self {
        Self {
            ...
            fallback_localizer: AtomicU8::new(0),
    ...
    pub(crate) fn apply_config(&self, config: &ScannerTuningConfig) {
        self.set_fallback_hs(config.fallback_hs);
        self.set_hs_prefilter_max_len(config.hs_prefilter_max_len);
        self.set_fallback_anchor_mode(config.fallback_anchor);
        self.set_fallback_homoglyph_gate(config.homoglyph_gate);
        self.set_homoglyph_ascii_skip(config.homoglyph_ascii_skip);
        self.set_fallback_reverse(config.fallback_reverse);
        self.set_prefilter_truncate(config.prefilter_truncate);
        self.set_fallback_prefix_gate(config.fallback_prefix_gate);
        ...
        self.set_fallback_localizer(config.fallback_localizer);
    ...
    pub(crate) fn set_fallback_hs(&self, mode: Option<bool>) {
    ...
    pub(crate) fn set_fallback_anchor_mode(&self, mode: Option<bool>) {
    ...
    pub(crate) fn set_fallback_homoglyph_gate(&self, mode: Option<bool>) {
    ...
    pub(crate) fn set_fallback_reverse(&self, mode: Option<bool>) {
    ...
    pub(crate) fn set_fallback_prefix_gate(&self, mode: Option<bool>) {
    ...
    // ── Fallback plain-pattern localizer ──────────────────────────────────

    /// Override fallback plain-pattern localization (test/diagnostic).
    pub(crate) fn set_fallback_localizer(&self, mode: Option<bool>) {
        self.fallback_localizer
            .store(encode_override(mode), Relaxed);
    }

    /// Whether the localized plain-pattern fallback path is enabled. Default
    ...
    pub(crate) fn fallback_localizer_enabled(&self) -> bool {
        match self.fallback_localizer.load(Relaxed) {
            1 => true,
            2 => false,
