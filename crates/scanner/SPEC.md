...
- Detector-owned candidate grammar distinguishes provider assignments and command arguments from identifier members, nested option names, truncated JWT fixtures, sibling-provider prefixes, and scanner-rule syntax without consulting repository identity or path-specific exclusions.
...
- Structured source semantics run only after candidate retrieval and inspect at most a 64 KiB candidate window. JSON, JSONL, TOML, YAML, dotenv, and INI classification yields a typed role, parser confidence, exact candidate/value spans, and bounded borrowed key-path spans. Invalid or truncated candidate syntax and unsupported, over-nested, or over-budget input produce `unknown` with abstaining confidence and cannot suppress a finding. The compact role and confidence occupy existing sidecar padding; `CandidateProvenance` remains at most 16 bytes.
