...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut wv: Vec<_> = by_wrapper.into_iter().collect();
    wv.sort_by(|a, b| b.1.cmp(&a.1));
    for (w, n) in &wv {
        println!("{n:4} {w}");
    }
    println!("--- top bare-fail (C) detectors ---");
    let mut bv: Vec<_> = bare_fail_detectors.into_iter().collect();
    bv.sort_by(|a, b| b.1.cmp(&a.1));
    for (d, n) in bv.iter().take(20) {
        println!("{n:4} {d}");
    }
    println!("--- top E detectors ---");
    let mut ev: Vec<_> = e_detectors.into_iter().collect();
    ev.sort_by(|a, b| b.1.cmp(&a.1));
    for (d, n) in ev.iter().take(20) {
        println!("{n:4} {d}");
    }
