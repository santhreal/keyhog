//! Static classification of the keyword-gated fallback pattern set, sizing the
//! prize for regex-anchored fallback localization.
//!
...
//! union comment). This test reports how much of the fallback set carries such
//! a regex-proven anchor.
//!
//! Run:
//!   cargo test --profile release-fast -p keyhog-scanner --test \
//!     fallback_anchor_analysis -- --ignored --nocapture
...
fn fallback_anchor_prize() {
    let detectors = keyhog_core::load_detectors(&detector_dir()).expect("detectors");
    let scanner = CompiledScanner::compile(detectors).expect("compile");
    let (total, always_active, aae) = scanner.fallback_anchor_stats();
    eprintln!(
        "ANCHOR STATS: fallback_total={total} always_active={always_active} \
         always_active_eligible={aae} prefilter_remaining={}",
        always_active - aae
    );
    let fallback = scanner.fallback_pattern_diagnostics();
    ...
    eprintln!(
        "=== FALLBACK ANCHOR PRIZE ({} fallback patterns) ===",
        t.total
    );
    eprintln!(
        "  parse_fail           : {:>4} ({:>5.1}%)",
...
fn dump_xox_literal_cover() {
    ...
    let detectors = keyhog_core::load_detectors(&detector_dir()).expect("detectors");
    let scanner = CompiledScanner::compile(detectors).expect("compile");
    for (src, kw) in scanner.fallback_pattern_diagnostics() {
