...
fn fixed_corpus_surfaces_exactly_the_expected_detectors_on_simd() {
    ...
    let results = scanner
        .scan_chunks_with_backend(&corpus, ScanBackend::SimdCpu)
...
fn simd_and_cpu_fallback_produce_identical_match_records() {
    ...
    let cpu = records(
        &scanner
            .scan_chunks_with_backend(&corpus, ScanBackend::CpuFallback)
            .expect("selected backend scan succeeds"),
    );
...
fn gpu_produces_identical_complete_records_without_degrade() {
    ...
    let gpu = records(
        &scanner
            .scan_chunks_with_backend(&corpus, ScanBackend::GpuWgpu)
            .expect("selected backend scan succeeds"),
    );
