...
//! `scan_fallback_patterns` costs, WHICH detectors burn it, and are they
//! prefix-anchorable (localizable to windows) or no-literal whole-chunk walks?
//!
//! Run:
//!   KEYHOG_PROFILE_FALLBACK=1 cargo test --profile release-fast -p keyhog-scanner \
//!     --test fallback_pattern_profile -- --ignored --nocapture
...
fn chunk_of(bytes: &[u8], label: &str) -> Chunk {
    Chunk {
        ...
        metadata: ChunkMetadata {
            source_type: "fallback-profile".into(),
...
#[test]
#[ignore = "measurement; set KEYHOG_PROFILE_FALLBACK=1 and run with --ignored --nocapture"]
fn fallback_pattern_profile_mirror() {
    let detectors = keyhog_core::load_detectors(&detector_dir()).expect("detectors");
    let scanner = CompiledScanner::compile(detectors).expect("compile");

    let Some(root) = corpus_dir() else {
        eprintln!("fallback_pattern_profile: mirror corpus absent — skipping");
        return;
    };
    let files = collect_files(&root, 8000);
    eprintln!("fallback_pattern_profile: {} mirror files", files.len());
    ...
    scanner.fallback_profile_dump("warmup-discard");
    ...
    scanner.fallback_profile_dump("mirror-16kib");
}
