//! Differential gate for the fallback prefix-literal skip gate: skipping an
//! always-active fallback batch whose patterns ALL provably require a prefix
...
//!     -p keyhog-scanner --test fallback_prefix_gate_parity -- --nocapture
...
fn diff_panic(label: &str, on: &[(String, String, String)], off: &[(String, String, String)]) {
    ...
    panic!("fallback prefix gate changed findings on {label}");
}
...
#[test]
fn fallback_prefix_gate_parity_default() {
