...
fn discord_url(seed: usize) -> String {
    format!("https://discord.com/api/webhooks/123456789012345678/{}", alnum(68, seed))
}
...
fn slack_incoming_webhook_url_surfaces() {
    let url = slack_url(1);
    assert!(surfaces_under(&url, "slack-webhook-url", &url), "a Slack webhook URL must surface");
}
...
fn slack_webhook_url_in_yaml_value_surfaces() {
    ...
    assert!(surfaces(&text, &url), "a Slack webhook URL as a YAML value must surface");
}

#[test]
fn slack_webhook_url_anchored_assignment_surfaces() {
    let url = slack_url(3);
    assert!(surfaces(&format!("SLACK_WEBHOOK_URL={url}"), &url), "SLACK_WEBHOOK_URL= must surface");
}
...
fn discord_webhook_url_in_json_field_surfaces() {
    ...
    assert!(surfaces(&text, &url), "a Discord webhook URL in a JSON field must surface");
}

#[test]
fn zapier_webhook_url_surfaces() {
    let url = format!("https://hooks.zapier.com/hooks/catch/123456789/{}/", alnum(16, 6));
    assert!(surfaces_under(&url, "zapier-webhook-url", &url), "a Zapier webhook URL must surface");
}
...
fn github_webhook_secret_surfaces() {
    let sec = alnum(40, 9);
    assert!(
        surfaces_under(&format!("GITHUB_WEBHOOK_SECRET={sec}"), "github-webhook-secret", &sec),
...
fn shopify_webhook_secret_hex_surfaces() {
    let sec = hex(48, 11);
    assert!(
        surfaces_under(&format!("SHOPIFY_WEBHOOK_SECRET={sec}"), "shopify-webhook-secret", &sec),
...
fn slack_and_discord_webhooks_cosurface() {
    ...
    assert!(surfaces(&text, &s), "Slack webhook must surface alongside Discord");
    assert!(surfaces(&text, &d), "Discord webhook must surface alongside Slack");
}
...
fn stripe_and_github_webhook_secrets_cosurface() {
    ...
    assert!(surfaces(&text, &stripe), "Stripe secret must surface alongside GitHub");
    assert!(surfaces(&text, &gh), "GitHub secret must surface alongside Stripe");
}
...
fn slack_wrong_host_dot_io_does_not_fire() {
    ...
    let url = format!("https://hooks.slack.io/services/T{}/B{}/{}", alnum(9, 16), alnum(9, 17), alnum(24, 18));
    assert!(
...
fn slack_short_secret_segment_does_not_fire() {
    ...
    let url = format!("https://hooks.slack.com/services/T{}/B{}/{}", alnum(9, 19), alnum(9, 20), alnum(20, 21));
    assert!(
        !fires(&url, "slack-webhook-url"),
...
fn discord_wrong_host_does_not_fire() {
    let url = format!("https://example.com/api/webhooks/123456789012345678/{}", alnum(68, 22));
    assert!(
...
fn shopify_webhook_secret_non_hex_does_not_fire() {
    ...
    assert!(
        !fires(&format!("SHOPIFY_WEBHOOK_SECRET={sec}"), "shopify-webhook-secret"),
...
fn github_webhook_below_min_length_does_not_fire() {
    ...
    assert!(
        !fires(&format!("GITHUB_WEBHOOK_SECRET={sec}"), "github-webhook-secret"),
...
fn discordapp_com_host_alias_surfaces() {
    let url = format!("https://discordapp.com/api/webhooks/123456789012345678/{}", alnum(68, 25));
    assert!(
        surfaces_under(&url, "discord-webhook-credentials", &url),
