...
fn chunk_boundary_discord_bot_split_reassembled() {
    ...
    let found = results
        .iter()
        .flatten()
        .any(|m| m.detector_id.as_ref() == "discord-bot-token" && m.credential.as_ref() == secret);
    assert!(
        found,
        "discord-bot-token split across chunk seam must reassemble"
    );
}
