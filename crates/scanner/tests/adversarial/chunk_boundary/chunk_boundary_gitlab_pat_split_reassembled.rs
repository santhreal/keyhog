//! R5-T-SCAN engine chunk boundary: gitlab pat split reassembled.
...
fn chunk_boundary_gitlab_pat_split_reassembled() {
    ...
    let secret = "glpat-aB3kQp7VbT2hYRzNcMfW";
    let split = 10;
    let pad = "z\n".repeat(4096);
    ...
    assert!(
        found,
        "gitlab-personal-access-token split across chunk seam must reassemble"
    );
}
