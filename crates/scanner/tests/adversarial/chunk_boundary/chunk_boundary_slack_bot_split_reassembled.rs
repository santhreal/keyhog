//! R5-T-SCAN engine chunk boundary: slack bot split reassembled.
...
#[test]
fn chunk_boundary_slack_bot_split_reassembled() {
    ...
    let found = results.iter().flatten().any(|m| m.detector_id.as_ref() == "slack-bot-token" && m.credential.as_ref() == "xoxb-1234567890-1234567890123-abcdefghijklmnopqrstuvwx");
    assert!(found, "slack-bot-token split across chunk seam must reassemble");
}
