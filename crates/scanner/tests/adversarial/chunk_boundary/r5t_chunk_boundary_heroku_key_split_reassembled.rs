//! R5-T engine chunk boundary: heroku-api-key split across seam must reassemble.
...
#[test]
fn r5t_chunk_boundary_heroku_key_split_reassembled() {
    ...
    let found = results.iter().flatten().any(|m| m.detector_id.as_ref() == "heroku-api-key" && m.credential.as_ref() == secret);
    assert!(found, "heroku-api-key split across chunk seam must reassemble");
}
