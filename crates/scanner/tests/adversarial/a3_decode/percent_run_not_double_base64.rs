...
fn percent_encoded_blob_not_also_decoded_as_spurious_base64() {
    ...
    let base64_only = decoded.iter().all(|c| !c.metadata.source_type.ends_with("/base64") || c.data.contains('%'));
    assert!(base64_only || decoded.iter().any(|c| c.data.contains("AKIA")), "expect url decode, not spurious bare base64 of hex digits");
}
