...
fn r5_handwritten_twin_gap_vs_detector_load() {
    ...
    let handwritten = std::fs::read_dir(&adv)
        ...
        .count();
    assert!(handwritten < loaded, "KH-GAP-155: expected handwritten twin gap — {handwritten}/{loaded} covered");
    assert!(handwritten >= 50, "KH-GAP-155: R5 floor requires >=50 handwritten near-miss twins, got {handwritten}");
}
