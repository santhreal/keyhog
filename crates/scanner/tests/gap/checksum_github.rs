...
//! (`GithubClassicPatValidator`), routed through the scanner testing surface.
//!
...
use keyhog_scanner::testing::checksum::{
    github_classic_pat_with_checksum, validate_checksum, ChecksumResult, GithubClassicPatValidator,
};

// ── Reference replica of the source's CRC32 / base62 (proptest oracle) ──────
// This is an independent oracle for golden vectors only. Valid test fixtures
// are minted through `github_classic_pat_with_checksum`, so the test does not
// use a private production-algorithm copy as its fixture owner.
...
fn valid_token(entropy: &str) -> String {
    github_classic_pat_with_checksum(entropy)
}
