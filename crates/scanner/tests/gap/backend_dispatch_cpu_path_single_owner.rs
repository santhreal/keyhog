...
fn cpu_scan_and_boundary_path_has_single_owner() {
    ...
    let direct_traversal = src.contains("chunks\n                    .iter()\n                    .enumerate()\n                    .map");
    let lane_traversal = src.contains("work_lanes\n                    .lanes()\n                    .iter()\n                    .map");
    assert!(
        direct_traversal && lane_traversal,
        "the CPU owner must contain exactly its direct and coalesced-lane traversals"
    );
