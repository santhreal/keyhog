...
fn keyword_credential_anchor_outputs_unchanged_by_lazy_lowercase() {
    ...
    assert!(!is_anchor("none (high-entropy)"), "no-keyword sentinel is not an anchor");
    ...
    assert!(is_anchor("client_secret"), "client_secret is a credential anchor");
    assert!(is_anchor("bearer"), "bearer is a credential anchor (explicit)");

    // Non-credential keywords reach the lowercase path and reject.
    assert!(!is_anchor("hello"), "a plain word is not a credential anchor");
    assert!(!is_anchor("qwerty"), "a plain word is not a credential anchor");
    ...
    let return_pos = src.find("return true;").expect("normalize early-return present");
    let lower_pos = src
