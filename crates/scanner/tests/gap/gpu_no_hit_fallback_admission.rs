...
//! real active fallback set, so a chunk that fires no Hyperscan literal but does
...
//! `has_active_fallback_patterns_for_chunk` prefilter. The old per-backend
...
fn no_hit_admission_consults_active_fallback_set() {
    let scan = scanner_source("engine/scan.rs");

    // The shared no-hit admission gate must consult the real active fallback set
    // FIRST (the exact, cheap necessary condition for an anchorless match).
    assert!(
        scan.contains("self.has_active_fallback_patterns_for_chunk(&chunk.data)"),
        "should_scan_no_hit_chunk must probe the active fallback set to preserve \
