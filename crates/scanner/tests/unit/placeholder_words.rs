...
fn parse_vocab_rejects_uppercase_marker_in_file() {
    let err = parse_vocab(&toml_with_markers("marker_substrings = [\"EXAMPLE\"]\n")).unwrap_err();
...
fn parse_vocab_rejects_non_ascii_marker() {
    let err = parse_vocab(&toml_with_markers("marker_substrings = [\"caf\u{e9}\"]\n")).unwrap_err();
    assert!(err.contains("alphanumeric"), "got: {err}");
}
...
fn parse_placeholder_words_wrapper_returns_only_words() {
    let words = parse_placeholder_words(&toml_with_markers("marker_substrings = [\"example\"]\n"))
        .expect("valid");
...
fn parse_vocab_rejects_duplicate_entropy_marker() {
    let err = parse_vocab(&toml_with_entropy("exact_values = [\"null\", \"null\"]\n")).unwrap_err();
    assert!(err.contains("duplicate"), "got: {err}");
}
