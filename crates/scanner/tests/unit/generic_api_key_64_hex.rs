...
fn test_generic_api_key_64_hex_policy() {
    ...
    let payload =
        r#"{"api_secret_key": "c3ab8ff13720e8ad9047dd39466b3c8974e592c2fa383d4a97601b1a7d6e492b"}"#;
    ...
    assert!(
        ...
        "api_secret_key 64-hex JSON field must match"
    );
...
fn test_generic_api_secret_key_64_hex_assignment_policy() {
    ...
    let target_spec = specs
        ...
        .expect("generic-api-key detector spec present");
    println!("GENERIC API KEY KEYWORDS: {:?}", target_spec.keywords);
    println!(
        "GENERIC API KEY CANONICAL HEX: {:?}",
        target_spec.canonical_hex_key_material
    );
    let scanner = CompiledScanner::compile(specs).expect("compile scanner");
    let payload =
        "api_secret_key = \"c3ab8ff13720e8ad9047dd39466b3c8974e592c2fa383d4a97601b1a7d6e492b\"\n";
    let mut chunk = Chunk::from(payload);
    ...
    assert!(
        !matches.is_empty() && !matches[0].is_empty(),
        "api_secret_key 64-hex assignment must match"
    );
    let generic_match = &matches[0][0];
    assert_eq!(generic_match.detector_id.as_ref(), "generic-api-key");
    assert_eq!(
