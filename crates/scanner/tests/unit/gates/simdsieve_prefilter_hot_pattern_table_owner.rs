...
fn hot_pattern_slot_metadata_has_one_owner() {
    ...
    assert!(
        ...
        "hot prefixes, min lengths, and identity metadata must be generated from one simdsieve table with a compile-time length guard"
    );
    assert!(
        ...
        "engine hot-pattern dispatch must consume the shared min-length table and fail loud on slot drift, not keep a local fallback table"
    );
}
