...
fn gpu_moe_honors_disabled_policy_before_adapter_probe() {
    ...
    let policy_gate = body
        .find("crate::gpu::gpu_disabled_by_policy()")
        .expect("GPU MoE backend checks resolved GPU runtime policy");
    let adapter_probe = body
        .find("dispatch_moe_batch(features, readback_timeout)")
        .expect("GPU MoE backend still dispatches through the adapter owner");

    assert!(
        policy_gate < adapter_probe,
        "--no-gpu must return to CPU MoE before the dispatch path can probe a \
