//! Gate `confidence::signals`: no .unwrap( / .expect( in production source lines.

#[test]
fn confidence_signals_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "confidence::signals: unwrap/expect in production source at {:?}",
