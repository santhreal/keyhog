...
fn engine_windowed_non_empty() {
    ...
    assert!(
        coalesced_src.contains("self.scan_windowed_with_triggered(chunk, &triggered, None)"),
        "engine::scan_coalesced: triggered large chunks must route through scan_windowed_with_triggered"
    );
}
