//! Gate `confidence::prefixes`: no .unwrap( / .expect( in production source lines.

#[test]
fn confidence_prefixes_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "confidence::prefixes: unwrap/expect in production source at {:?}",
