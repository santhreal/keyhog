//! Gate `multiline::config`: no .unwrap( / .expect( in production source lines.

#[test]
fn multiline_config_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "multiline::config: unwrap/expect in production source at {:?}",
