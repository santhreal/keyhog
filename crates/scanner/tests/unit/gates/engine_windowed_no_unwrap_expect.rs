//! Gate `engine::windowed`: no .unwrap( / .expect( in production source lines.

#[test]
fn engine_windowed_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "engine::windowed: unwrap/expect in production source at {:?}",
