//! Gate `engine::fallback_entropy`: no .unwrap( / .expect( in production source lines.

#[test]
fn engine_fallback_entropy_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "engine::fallback_entropy: unwrap/expect in production source at {:?}",
