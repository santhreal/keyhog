//! Gate `multiline::fragment_cache`: no .unwrap( / .expect( in production source lines.

#[test]
fn multiline_fragment_cache_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "multiline::fragment_cache: unwrap/expect in production source at {:?}",
