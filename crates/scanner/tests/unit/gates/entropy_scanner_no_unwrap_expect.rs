//! Gate `entropy::scanner`: no .unwrap( / .expect( in production source lines.

#[test]
fn entropy_scanner_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "entropy::scanner: unwrap/expect in production source at {:?}",
