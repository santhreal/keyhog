//! Gate `multiline::structural`: no .unwrap( / .expect( in production source lines.

#[test]
fn multiline_structural_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "multiline::structural: unwrap/expect in production source at {:?}",
