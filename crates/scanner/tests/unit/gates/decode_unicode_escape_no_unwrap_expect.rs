//! Gate `decode::unicode_escape`: no .unwrap( / .expect( in production source lines.

#[test]
fn decode_unicode_escape_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "decode::unicode_escape: unwrap/expect in production source at {:?}",
