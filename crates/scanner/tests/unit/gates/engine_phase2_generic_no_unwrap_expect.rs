//! Gate `engine::phase2_generic`: no .unwrap( / .expect( in production source lines.

#[test]
fn engine_phase2_generic_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "engine::phase2_generic: unwrap/expect in production source at {:?}",
