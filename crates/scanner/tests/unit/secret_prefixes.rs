...
fn parse_preserves_case_verbatim() {
    ...
    let parsed = parse_multiline_secret_prefixes(&toml_with("\"HRKU-\", \"sk-proj-\"")).unwrap();
...
fn ac_built_from_tier_b_is_case_sensitive() {
    ...
    let ac = aho_corasick::AhoCorasick::new(multiline_secret_prefixes().iter().map(String::as_str))
