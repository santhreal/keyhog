...
fn json_unescape_splice_preserves_key_anchor() {
    ...
    let spliced = decoded.iter().any(|c| c.data.contains("api_key") && c.data.contains("sk-proj-abc123"));
    assert!(spliced, "json decoder must splice unescaped credential near key anchor");
}
