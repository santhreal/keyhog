...
fn hex_splice_preserves_companion_anchor() {
    ...
    let spliced = decoded.iter().any(|c| c.data.contains("api_key") && c.data.contains(secret));
    assert!(spliced, "hex splice must keep api_key anchor next to decoded secret");
}
