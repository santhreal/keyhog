...
fn html_entity_decode_splices_into_parent() {
    ...
    let spliced = decoded.iter().any(|c| c.data.contains("password") && c.data.contains("<secret>"));
    assert!(spliced, "html entity decode must splice unescaped text near assignment key");
}
