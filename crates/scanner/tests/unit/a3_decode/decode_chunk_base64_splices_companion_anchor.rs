...
fn base64_splice_preserves_companion_anchor() {
    ...
    let spliced = decoded.iter().any(|c| {
        c.data.contains("aws_secret_access_key") && c.data.contains("sk-proj-abc123")
    });
    assert!(spliced, "decoded chunk must splice secret adjacent to assignment anchor");
}
