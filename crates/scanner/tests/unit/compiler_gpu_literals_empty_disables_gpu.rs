...
#[test]
fn compiler_gpu_literals_append_phase2_keywords_after_detector_literals() {
    let literals = vec!["GhP_".into()];
    let phase2_keywords = vec!["PhaseTwoKey".into()];
    let built = build_gpu_literals(&literals, &phase2_keywords).expect("gpu literals");

    assert_eq!(
        built.as_ref(),
        &vec![b"ghp_".to_vec(), b"phasetwokey".to_vec()],
        "GPU literal rows must keep detector literals first, then phase2 keywords"
    );
}
