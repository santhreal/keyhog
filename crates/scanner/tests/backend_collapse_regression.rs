...
//! no real scan. `KEYHOG_NO_GPU` is process-global and explicit backend pins
...
/// Run `body` with `KEYHOG_NO_GPU` and an optional race-free backend test
/// override, restoring state on exit.
fn with_env<R>(no_gpu: Option<&str>, backend: Option<&str>, body: impl FnOnce() -> R) -> R {
    let _g = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    let prior_no_gpu = std::env::var("KEYHOG_NO_GPU").ok();
    unsafe {
        match no_gpu {
            Some(v) => std::env::set_var("KEYHOG_NO_GPU", v),
            None => std::env::remove_var("KEYHOG_NO_GPU"),
        }
    }
...
fn select_backend_routes_cpu_tier_through_the_shared_helper() {
    ...
    with_env(Some("1"), None, || {
        for &(hs, simd) in &[(true, true), (true, false), (false, true), (false, false)] {
            let caps = caps_gpu(hs, simd); // GPU present but KEYHOG_NO_GPU=1.
            assert_eq!(
                select_backend(&caps, SIXTEEN_MIB, 5_000),
                cpu_tier_backend(&caps),
                "select_backend under KEYHOG_NO_GPU=1 must equal cpu_tier_backend (hs={hs} simd={simd})"
            );
            assert_eq!(
                select_backend_for_batch(&caps, SIXTEEN_MIB, 5_000, SIXTEEN_MIB),
                cpu_tier_backend(&caps),
                "select_backend_for_batch under KEYHOG_NO_GPU=1 must equal cpu_tier_backend (hs={hs} simd={simd})"
            );
        }
    });
}
