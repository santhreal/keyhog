...
fn decode_chunk_recovers_credential_via_registry_selected_shift() {
    ...
    let out = CaesarDecoder
        .decode_chunk(&chunk(&body, "src", Some("creds.env")))
...
fn decode_chunk_emits_nothing_when_registry_selects_no_shift() {
    ...
    let out = CaesarDecoder
        .decode_chunk(&chunk(body, "src", None))
        .expect("should decode successfully");
...
fn decode_chunk_enforces_min_caesar_len_boundary() {
    ...
    let short_out = CaesarDecoder
        .decode_chunk(&chunk(short_body, "src", None))
        .expect("should decode successfully");
    assert_eq!(
        short_out.len(),
        0,
    ...
    let long_out = CaesarDecoder
        .decode_chunk(&chunk(long_body, "src", None))
        .expect("should decode successfully");
    assert!(
...
fn decode_chunk_refuses_recursion_on_nested_caesar_source_type() {
    ...
    let body = format!("token = \"{AKIA_ENC_SHIFT3}\"\n");
    let out = CaesarDecoder
        .decode_chunk(&chunk(&body, "raw/base64/caesar", Some("creds.env")))
        .expect("should decode successfully");
    assert_eq!(
        out.len(),
        0,
