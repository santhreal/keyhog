...
fn scalar_path_matches_from_bytes_entry() {
    ...
    assert_eq!(
        AlphabetMask::from_bytes(&data),
        AlphabetMask::from_bytes_scalar(&data)
    );
}
...
fn avx2_body_matches_scalar_when_available() {
    ...
    let simd = unsafe { AlphabetMask::from_bytes_avx2(&data) };
    assert_eq!(simd, AlphabetMask::from_bytes_scalar(&data));
}
