...
fn a3_multiline_unit_files_are_declared() {
    ...
    let files = std::fs::read_dir(&dir)
        ...
        .collect::<BTreeSet<_>>();
    let mod_src = std::fs::read_to_string(dir.join("mod.rs")).expect("a3_multiline/mod.rs");
    ...
    assert_eq!(
        files, declared,
        "tests/unit/a3_multiline files must be declared in mod.rs so they compile and run"
    );
}
