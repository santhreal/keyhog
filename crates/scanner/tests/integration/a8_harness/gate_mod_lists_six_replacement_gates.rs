//! LR2-A8 harness integration: scanner gate-dir wiring coherence + retention floor.
//!
//! Replaces a brittle `assert_eq!(count, 6)` magic-constant tripwire with the
...
//!   2. RETENTION, the LR1-A8 migration created 6 replacement gates; that
...
/// LR1-A8 migration baseline for the scanner crate. A floor, not an exact count:
/// deletions below it fail, additions above it are fine.
const GATE_RETENTION_FLOOR: usize = 6;
...
fn gate_dir_files_are_wired_and_meet_floor() {
    ...
    assert_eq!(
        files, declared,
        "scanner tests/gate/: every gate .rs file must be wired in mod.rs (no dead/unwired gate)"
    );
    assert!(
        files.len() >= GATE_RETENTION_FLOOR,
        "scanner tests/gate/ must retain >= {GATE_RETENTION_FLOOR} replacement tests, found {}",
        files.len()
    );
}
