...
use keyhog_scanner::decode::{decode_chunk, register_decoder, Decoder};
...
impl Decoder for FanoutDecoder {
    ...
    fn decode_chunk(&self, chunk: &Chunk) -> Vec<Chunk> {
        ...
        for i in 0..FANOUT {
            ...
            out.push(Chunk {
                data: SensitiveString::new(format!("c9.tok.{i:05}.end")),
...
fn inert_root(path: &str) -> Chunk {
    Chunk {
        data: SensitiveString::new("alpha.bravo.charlie.delta.echo.foxtrot".to_string()),
        metadata: ChunkMetadata {
