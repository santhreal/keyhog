...
//!
//! The npm CRC32/base62 algorithm is replicated VERBATIM from
//! `crates/scanner/src/checksum/github.rs` so that "Valid" cases assert the
//! validator agrees with the documented algorithm, not a guessed string.

use keyhog_scanner::checksum::{
    validate_checksum, ChecksumResult, ChecksumValidator, GitlabTokenValidator, NpmTokenValidator,
    SlackTokenValidator, StripeTokenValidator,
};

// ───────────────────────── algorithm mirror (npm) ─────────────────────────
//
// Verbatim copy of github.rs `crc32` + `base62_encode_u32`. npm.rs calls these
// exact functions (`super::github::{base62_encode_u32, crc32}`), so a token
// built with this mirror is, by construction, the one the validator expects.
...
fn make_valid_npm(entropy: &str) -> String {
    ...
    assert_eq!(checksum.len(), 6, "base62(6) must be 6 chars");
...
fn npm_all_a_36_body_is_invalid() {
    ...
    let expected = base62_encode_u32(crc32(&[b'A'; 30]), 6);
    assert_ne!(
        expected, "AAAAAA",
