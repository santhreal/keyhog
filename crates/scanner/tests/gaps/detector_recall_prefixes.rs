...
//!     6-char base62 equals `base62(crc32(body), 6)`. We recompute that CRC the
//!     same way `checksum/github.rs` does (`crc32` + `base62_encode_u32`) so the
//!     token we plant is the exact thing the validator calls `Valid`.
...
// ── CRC32 + base62: faithful copies of crates/scanner/src/checksum/github.rs ──
//
// We recompute the embedded checksum exactly as the validator does so the
// tokens we mint are the *same* values `validate_checksum` adjudicates `Valid`.
// (We cannot call the crate's `crc32` / `base62_encode_u32` — they are
// `pub(super)`. Re-deriving here is the truthful move: if these helpers ever
// drift from the source, the `crc32_*` self-checks below catch it.)
...
fn mint_crc_token(prefix: &str, body30: &str) -> String {
    ...
    let cksum = base62_encode_u32(crc32(body30.as_bytes()), 6);
    assert_eq!(cksum.len(), 6, "base62 CRC is exactly 6 chars");
    format!("{prefix}{body30}{cksum}")
}
...
fn mint_github_fine_grained(left22: &str, right_body53: &str) -> String {
    assert_eq!(left22.len(), 22);
    assert_eq!(right_body53.len(), 53);
    let cksum = base62_encode_u32(crc32(right_body53.as_bytes()), 6);
    format!("github_pat_{left22}_{right_body53}{cksum}")
}

// ── CRC self-checks: prove our re-derivation matches the published algorithm ─
...
fn proptest_minted_ghp_tokens_are_always_valid() {
    ...
    for _ in 0..2_000 {
        ...
        for _ in 0..30 {
            ...
            body.push(BASE62_DIGITS[idx] as char);
        }
