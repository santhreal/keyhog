//! Integration tests for the decode-through pipeline (`decode::decode_chunk`)
...
use keyhog_scanner::decode::caesar::{caesar_shift, is_source_code_path, looks_credential_shaped};
...
use keyhog_scanner::decode::{
    base64_decode, decode_chunk, find_base64_strings, hex_decode, z85_decode,
};
...
fn pipeline_surfaces_base64_single_layer() {
    ...
    assert!(hit.data.as_str().contains("aws_secret"));
    // The original encoded blob must be replaced by the decoded text.
    assert!(!hit.data.as_str().contains("QUtJQUlPU0ZPRE5ON0VYQU1QTEU"));
...
fn pipeline_surfaces_hex_single_layer() {
    ...
    assert!(hit.data.as_str().contains("apikey"));
}
...
fn pipeline_surfaces_caesar_single_layer() {
    ...
    assert!(
        out.iter()
            .any(|d| d.data.as_str() == SECRET && d.metadata.source_type.ends_with("/caesar")),
...
fn pipeline_never_reemits_the_root_chunk() {
    ...
    let out = decode_chunk(&c, 3, false, None, None);
    assert!(
        out.iter().all(|d| d.data.as_str() != root),
...
fn pipeline_decoded_chunks_are_unique_by_data() {
    ...
    let out = decode_chunk(&c, 3, false, None, None);
    let mut datas: Vec<&str> = out.iter().map(|c| c.data.as_str()).collect();
