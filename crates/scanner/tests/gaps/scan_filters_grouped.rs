...
//! `KEYWORD[=:\s"']+(VALUE)`, the engine passes `match_start` = the WHOLE-match
//! start (i.e. the keyword), not the credential start, into the extension
//! helpers. A naive `data[match_start..end]` slice would therefore prepend the
//! keyword (`segment_write_key=`, `HOMEBREW_GITHUB_API_TOKEN=`, …) onto the
//! captured credential — credential corruption. The helpers defend against this
//! by re-deriving the slice from the credential's OWN pointer offset within
//! `data`. These tests assert the surfaced credential bytes are EXACTLY the
//! secret with no keyword prefix and correctly recovered base64 padding.
//!
...
/// Mirror of the scanner's `PLACEHOLDER_WORDS` gate (case-insensitive). A body
