...
use keyhog_scanner::confidence::{
    ...
    char_diversity, compute_confidence, compute_confidence_with_threshold,
    contains_placeholder_word, is_sensitive_path, known_prefix_confidence_floor, max_repeat_run,
    ConfidenceSignals, KNOWN_PREFIXES,
};
use keyhog_scanner::testing::finalize_confidence;
...
fn unknown_prefix_has_no_floor() {
    assert_eq!(known_prefix_confidence_floor("randomtokenwithnoprefix12345"), None);
}
...
fn degenerate_repeat_blocks_floor() {
    ...
    assert_eq!(
        known_prefix_confidence_floor("ghp_XXXXXXXXXXXXXXXX"),
        None
    );
}
...
fn known_prefixes_table_contains_core_providers() {
    for p in ["ghp_", "AKIA", "sk_live_", "xoxb-", "npm_", "glpat-"] {
        assert!(KNOWN_PREFIXES.contains(&p), "{p} missing from KNOWN_PREFIXES");
    }
}
...
fn post_ml_named_keeps_small_alphabet_secret() {
    ...
    assert!((out - 0.9).abs() < 1e-9, "named hex should survive, got {out}");
}
...
fn path_penalty_skipped_for_production_path() {
    ...
    assert!((out - 0.8).abs() < 1e-9, "non-test path unchanged, got {out}");
}
...
fn calibration_output_in_unit_range() {
    ...
    assert!((0.0..=1.0).contains(&out), "calibrated score out of range: {out}");
}
