...
//! (the `AlwaysActiveFallbackPrefilter` RegexSet path), which profiling put at
//! 58-74% of phase-2. That is the set this offload targets.
//!
//! Run: cargo test -p keyhog-scanner --release --features gpu \
//!        --test fallback_gpu_throughput -- --ignored --nocapture
...
/// Single-thread CPU baseline: the `AlwaysActiveFallbackPrefilter` shape — batched
...
fn fallback_gpu_vs_single_thread_cpu_16k() {
