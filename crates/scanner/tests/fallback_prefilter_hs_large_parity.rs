...
//!
//! `fallback_prefilter_hs_findings_parity` only covers ≤4 KiB mirror files, so
...
fn build_large_corpus() -> String {
    ...
    while buf.len() < 1_200_000 {
        ...
        if i % 2 == 0 {
            ...
            buf.push_str(&format!(
                "gf_{i} = \"google forms api key zzzz{i:028}\"\n"
            ));
        }
...
// `#[ignore]` to match `fallback_prefilter_hs_findings_parity`: the RegexSet
...
//     --test fallback_prefilter_hs_large_parity -- --ignored --nocapture
#[test]
