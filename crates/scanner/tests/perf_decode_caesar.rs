...
use keyhog_scanner::decode::caesar::CaesarDecoder;
use keyhog_scanner::decode::{decode_chunk, Decoder};
...
fn caesar_runfree_chunk_must_be_gated_not_fanned_out() {
    ...
    let bytes = chunk.data.as_str().len();
    let data = chunk.data.as_str().as_bytes().to_vec();
...
fn url_html_escape_wrapped_credential_still_decodes() {
    ...
    let url_ok = url_out.iter().any(|c| c.data.as_str().contains(aws));
    ...
    let html_ok = html_out.iter().any(|c| c.data.as_str().contains(aws));
    ...
    let hx_ok = hx_out.iter().any(|c| c.data.as_str().contains(aws));
