...
fn scan_both(scanner: &CompiledScanner, chunk: &Chunk) -> (Vec<Key>, Vec<Key>) {
    ...
    scanner.tuning().set_homoglyph_ascii_skip(Some(false));
    scanner.clear_fragment_cache();
    let off = canonical(&scanner.scan_chunks_with_backend(
        std::slice::from_ref(chunk),
        ScanBackend::CpuFallback,
    ));
    scanner.tuning().set_homoglyph_ascii_skip(Some(true));
    scanner.clear_fragment_cache();
    let on = canonical(&scanner.scan_chunks_with_backend(
        std::slice::from_ref(chunk),
        ScanBackend::CpuFallback,
    ));
    scanner.tuning().set_homoglyph_ascii_skip(None);
    (on, off)
}
