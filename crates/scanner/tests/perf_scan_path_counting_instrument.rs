...
fn scan_path_counting_instrument_measures_allocations_and_finding_parity() {
    ...
    assert_eq!(allocs1, allocs2, "allocation count must be deterministic across identical scans");
    assert_eq!(total_bytes1, total_bytes2, "total allocated bytes must be deterministic");
    assert_eq!(peak1, peak2, "peak footprint must be deterministic");
    assert!(peak1 <= 10 * 1024 * 1024, "peak memory during scan must be well within 10MB limit (got {} bytes)", peak1);
}
