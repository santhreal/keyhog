...
- Detector-owned grammars keep MongoDB Atlas key pairs and command-line password arguments across shell, Dockerfile, PowerShell, CI, and programmatic literal contexts while rejecting Atlas identifiers, nested password-option names, OpenAI and Helicone sibling shapes, and scanner-rule syntax. JWT detection retains its existing short-signature recall floor; provider-owned Scalr context applies a narrower 20-character floor. Known-prefix candidates used as assignment keys no longer absorb a quoted `=` separator as base64 padding. These decisions do not use repository or path exclusions.
