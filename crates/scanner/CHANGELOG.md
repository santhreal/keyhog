...
- Detector TOML accepts typed capture, anchor, allowed-source, and required-evidence roles. Omitted declarations preserve current findings. Detector-plan schema version 3 persists the resolved policy, rejects stale sections, and `explain` renders the policy.
