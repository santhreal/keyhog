...
fn non_success_get_is_counted_unreadable() {
    ...
    assert_eq!(
        rows.len(),
        1,
        "a non-success S3 object GET must surface one source error"
    );
    let err = rows[0]
        .as_ref()
        .expect_err("failed S3 object GET must be an error row");
    assert!(
        err.to_string().contains("GET returned 403")
            && err.to_string().contains("object was not scanned"),
        "error should name the unscanned S3 object, got {err}"
    );
...
fn truncated_listing_without_token_is_counted_source_truncated() {
    ...
    assert_eq!(
        errors.len(),
        1,
        "truncated listing without token must surface one source error"
    );
    let err = errors[0].to_string();
    assert!(
        err.contains("source scan was truncated")
