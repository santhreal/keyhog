//! Zip slip variant `/tmp/escape.env` must not surface extracted secrets.
...
#[test]
fn zip_slip_leading_slash_entry_not_extracted() {
    ...
    zip.start_file("/tmp/escape.env", opts).expect("start slip");
