//! Zip slip variant `\\server\share\secret.env` must not surface extracted secrets.
...
#[test]
fn zip_slip_windows_unc_path_not_extracted() {
    ...
    zip.start_file("\\\\server\\share\\secret.env", opts).expect("start slip");
