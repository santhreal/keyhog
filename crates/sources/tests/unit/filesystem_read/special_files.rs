...
use std::path::Path;
    ...
    fn read_file_safe_refuses_dev_null() {
        assert!(TestApi.read_file_safe_capped(Path::new("/dev/null"), 0).is_err());
    }
    ...
    fn open_file_safe_accepts_regular_file() {
        ...
        let mut file = TestApi.open_file_safe(&path).expect("regular file must open");
    ...
    fn open_file_safe_accepts_empty_regular_file() {
        ...
        let mut file = TestApi.open_file_safe(&path).expect("empty regular file must open");
        let mut s = String::new();
        file.read_to_string(&mut s).unwrap();
        assert_eq!(s, "");
    }
