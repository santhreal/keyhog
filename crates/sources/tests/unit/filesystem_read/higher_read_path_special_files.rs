...
use std::path::Path;
    ...
    fn buffered_refuses_dev_null_returns_none() {
        assert!(TestApi.read_file_buffered_text(Path::new("/dev/null"), 0).is_none());
    }
    ...
    fn compressed_input_refuses_directory_none() {
        let dir = tempfile::tempdir().unwrap();
        assert!(TestApi.read_file_for_compressed_input(dir.path(), CAP).is_none());
    }
    ...
    fn compressed_input_refuses_dev_null_none() {
        assert!(TestApi.read_file_for_compressed_input(Path::new("/dev/null"), CAP).is_none());
    }
    ...
    fn compressed_input_regular_file_returns_bytes() {
        ...
        let bytes = TestApi.read_file_for_compressed_input(&path, CAP).expect("regular file must read");
    ...
    fn windowed_refuses_dev_null_zero_windows() {
        assert_eq!(TestApi.read_file_windowed_mmap_len(Path::new("/dev/null"), 1024, 32), Some(0));
    }
    ...
    fn windowed_regular_file_returns_windows() {
        ...
        let n = TestApi.read_file_windowed_mmap_len(&path, 1024, 32).expect("regular file must mmap");
        assert!(
            n > 0,
    ...
    fn mmap_regular_text_file_returns_exact_text() {
        ...
        assert_eq!(TestApi.read_file_mmap(&path).as_deref(), Some("AKIAIOSFODNN7EXAMPLE\n"));
    }
    ...
    fn safe_capped_refuses_dev_null_err() {
        assert!(TestApi.read_file_safe_capped(Path::new("/dev/null"), CAP).is_err());
    }
