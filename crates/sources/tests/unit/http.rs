...
use keyhog_sources::{
    ...
    testing::{http_async_client_builder, http_effective_proxy},
};
...
fn http_client_config_effective_proxy_honors_explicit_off() {
    ...
    assert_eq!(http_effective_proxy(&config).as_deref(), Some("off"));
}

#[test]
fn http_client_config_default_builds_async_client() {
    let config = HttpClientConfig::default();
    let builder = http_async_client_builder(&config).expect("default config must build");
