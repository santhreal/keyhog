...
fn validate_extension_accepts_plain_lowercase_and_rejects_dot_or_path_separators() {
    ...
    assert!(validate_rule_value_for_test("extensions", ".exe", "extension")
        .is_err());
    assert!(validate_rule_value_for_test("extensions", "a/b", "extension")
        .is_err());
    assert!(validate_rule_value_for_test("extensions", "a\\b", "extension")
        .is_err());
    assert!(validate_rule_value_for_test("extensions", "PNG", "extension")
        .is_err());
}
...
fn validate_path_segment_and_filename_reject_path_separators_and_control_chars() {
    for kind in ["path_segment", "filename"] {
        ...
        assert!(validate_rule_value_for_test("dirs", "a/b", kind)
            .is_err());
        assert!(validate_rule_value_for_test("dirs", "a\\b", kind)
            .is_err());
        assert!(validate_rule_value_for_test("dirs", "bad\0dir", kind)
            .is_err());
    }
}
...
fn validate_suffix_requires_leading_dot_and_no_path_separators() {
    ...
    assert!(validate_rule_value_for_test("suffixes", "min", "suffix")
        .is_err());
    assert!(validate_rule_value_for_test("suffixes", ".a/b", "suffix")
        .is_err());
}
...
fn validate_infix_requires_leading_and_trailing_dot_and_no_path_separators() {
    ...
    assert!(validate_rule_value_for_test("infixes", "chunk", "infix")
        .is_err());
    assert!(validate_rule_value_for_test("infixes", ".chunk", "infix")
        .is_err());
    assert!(validate_rule_value_for_test("infixes", "chunk.", "infix")
        .is_err());
    assert!(validate_rule_value_for_test("infixes", ".a/b.", "infix")
        .is_err());
}

#[test]
fn validate_rule_value_rejects_empty_whitespace_uppercase_and_control_chars() {
    assert!(validate_rule_value_for_test("extensions", "", "extension")
        .is_err());
    assert!(validate_rule_value_for_test("extensions", "   ", "extension")
        .is_err());
    assert!(validate_rule_value_for_test("extensions", "PNG", "extension")
