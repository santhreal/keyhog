//! Gate `filesystem::read`: no .unwrap( / .expect( in production source lines.

#[test]
fn filesystem_read_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "filesystem::read: unwrap/expect in production source at {:?}",
