//! Gate `binary::sections`: no .unwrap( / .expect( in production source lines.

#[test]
fn binary_sections_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "binary::sections: unwrap/expect in production source at {:?}",
