...
//!   density rule.
//! * `stream_git_blobs` (source.rs:97) — `git log --all` over every ref, gix
//!   tree walk, `seen_blobs`/`seen_commits` dedup, the
//!   `node_modules|target|.git|__pycache__|dist|build|vendor` exact-name skip,
...
fn each_excluded_dir_name_is_skipped() {
    // All seven exact names in the skip list must drop their subtrees.
    for dirname in [
        "node_modules",
        "target",
        "__pycache__",
...
fn excluded_name_match_is_exact_not_prefix() {
    // The skip check uses `name == b"vendor"` (exact). A directory named
    // "vendored" or "vendor_libs" is NOT excluded, and a directory named
...
fn excluded_name_also_skips_plain_files_not_just_dirs() {
    // The name check runs BEFORE the tree/blob discrimination (source.rs:424),
    // so a regular FILE named exactly "vendor" or "build" is skipped too.
    let (_t, repo) = init_repo();
    commit_file(
        &repo,
        "vendor",
