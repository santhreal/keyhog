...
fn mmap_toctou_sanity_cap_in_read() {
    ...
    assert!(
        raw_src.contains("let meta = match file.metadata()")
            ...
            && raw_src.contains("MmapOptions::new().map(&file)")
            && raw_src.contains("mapped length exceeds sanity cap")
    ...
    assert!(
        window_src.contains("let meta = match file.metadata()")
            ...
            && window_src.contains("MmapOptions::new().map(&file)")
            && window_src.contains("mapped length {} exceeds the")
