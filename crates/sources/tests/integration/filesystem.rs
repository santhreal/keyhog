...
fn unreadable_subtree_does_not_abort_full_scan() {
    ...
    // The locked subtree surfaces a loud SourceError row (Law 10) instead of
    // aborting; the readable root file must still be scanned. Split so the error
...
fn merkle_skip_does_not_fire_when_size_drifts() {
    ...
    assert_eq!(TestApi.filesystem_skipped_count(&source), 0);
}
...
fn windowed_path_emits_multiple_chunks_with_overlap() {
    ...
    let source = TestApi.filesystem_with_window_config(dir.path().to_path_buf(), 128, 32);
    let chunks: Vec<_> = source.chunks().collect::<Result<Vec<_>, _>>().unwrap();
...
fn windowed_path_single_chunk_when_only_one_window_above_threshold() {
    ...
    let source = TestApi.filesystem_with_window_config(dir.path().to_path_buf(), 64, 8);
    let chunks: Vec<_> = source.chunks().collect::<Result<Vec<_>, _>>().unwrap();
    assert_eq!(chunks.len(), 2);
...
fn windowed_path_offsets_strictly_monotonic() {
    ...
    let source = TestApi.filesystem_with_window_config(dir.path().to_path_buf(), 256, 32);
    let chunks: Vec<_> = source.chunks().collect::<Result<Vec<_>, _>>().unwrap();
    assert!(
        chunks.len() >= 5,
