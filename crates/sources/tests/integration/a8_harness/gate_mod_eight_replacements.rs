//! LR2-A8 harness integration: sources gate-dir wiring coherence + retention floor.
//!
//! Replaces a brittle `assert_eq!(count, 8)` magic-constant tripwire with the
...
//!   2. RETENTION — the LR1-A8 migration created 8 replacement gates; that
...
/// LR1-A8 migration baseline for the sources crate. A floor, not an exact count:
/// deletions below it fail, additions above it are fine.
const GATE_RETENTION_FLOOR: usize = 8;
...
fn gate_dir_files_are_wired_and_meet_floor() {
    ...
    assert_eq!(
        files, declared,
        "sources tests/gate/: every gate .rs file must be wired in mod.rs (no dead/unwired gate)"
    );
    assert!(
        files.len() >= GATE_RETENTION_FLOOR,
        "sources tests/gate/ must retain >= {GATE_RETENTION_FLOOR} replacement tests, found {}",
        files.len()
    );
}
