...
pub(crate) fn read_to_cap_preserving_error(
    ...
    let read_limit = cap.saturating_add(1); // LAW10: recall-preserving (u64::MAX has no representable sentinel byte; reading up to the finite reader cap still reads all reachable bytes).
