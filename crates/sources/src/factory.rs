...
pub fn create_source_with_http_config_limits_and_policy(
    ...
    match name {
        // Every arm below decides the SAME way in every feature set: a missing
        // required parameter is rejected first (a fix-carrying error is actionable
        // in any build), then feature availability is decided WITHOUT binding a
        // constructor argument. The `Some(_)` arms bind nothing, so a
        // feature-disabled build compiles no pretend constructor path and needs no
        // `let _ = arg` discard marker to stay warning-free.
        "slack" => match params {
            None => Err(invalid_configuration("slack", "a token is required")),
            #[cfg(feature = "slack")]
            Some(token) => Ok(Box::new(
                crate::slack::SlackSource::new(token)
                    .with_http_config(http)
                    .with_limits(limits),
            )),
            #[cfg(not(feature = "slack"))]
            Some(_) => Err(feature_unavailable("slack", "slack")),
        },
        "docker" => match params {
            None => Err(invalid_configuration("docker", "an image name is required")),
            #[cfg(feature = "docker")]
            Some(image) => Ok(Box::new(
                crate::docker::DockerImageSource::new(image)
                    .with_limits(limits)
                    .with_default_excludes(respect_default_excludes),
            )),
            #[cfg(not(feature = "docker"))]
            Some(_) => Err(feature_unavailable("docker", "docker")),
        },
        ...
        "s3" => match params {
            None => Err(invalid_configuration("s3", "a bucket name is required")),
            #[cfg(feature = "s3")]
            Some(params) => {
                let fields = source_param_fields(params);
            ...
            #[cfg(not(feature = "s3"))]
            Some(_) => Err(feature_unavailable("s3", "s3")),
        },
        "gcs" => match params {
            None => Err(invalid_configuration("gcs", "a bucket name is required")),
            #[cfg(feature = "gcs")]
            Some(params) => {
                let fields = source_param_fields(params);
            ...
            #[cfg(not(feature = "gcs"))]
            Some(_) => Err(feature_unavailable("gcs", "gcs")),
        },
        "azure_blob" => Err(deprecated_source_name("azure_blob", "azure-blob")),
        "azure-blob" => match params {
            None => Err(invalid_configuration(
                "azure-blob",
            ...
            Some(params) => {
                ...
                if let Some(max_objects) = optional_usize_source_param("azure-blob", &fields, 2)? {
                    source = source.with_max_objects(max_objects);
                }
                Ok(Box::new(source))
            }
            #[cfg(not(feature = "azure"))]
            Some(_) => Err(feature_unavailable("azure-blob", "azure")),
        },
            ...
            Some(params) => {
                ...
                Ok(Box::new(
                    crate::web::WebSource::new(urls)
                        .with_http_config(http)
                        .with_limits(limits)
                        .with_autoroute_loopback_calibration(allow_autoroute_loopback_calibration),
                ))
            }
            #[cfg(not(feature = "web"))]
            Some(_) => Err(feature_unavailable("web", "web")),
        },
        "gitlab_group" => Err(deprecated_source_name("gitlab_group", "gitlab-group")),
        "gitlab-group" => match params {
            None => Err(invalid_configuration(
                "gitlab-group",
                "GROUP and TOKEN parameters are required; ENDPOINT is optional",
            )),
            #[cfg(feature = "gitlab")]
            Some(params) => Ok(Box::new(crate::gitlab_group::source_from_params(
                params,
                http,
                limits,
            ...
            Some(_) => Err(feature_unavailable("gitlab-group", "gitlab")),
        },
        "bitbucket_workspace" => Err(deprecated_source_name(
            "bitbucket_workspace",
            "bitbucket-workspace",
        )),
        "bitbucket-workspace" => match params {
            None => Err(invalid_configuration(
                "bitbucket-workspace",
                "WORKSPACE, USERNAME, and APP_PASSWORD parameters are required; ENDPOINT is optional",
            )),
            #[cfg(feature = "bitbucket")]
            Some(params) => Ok(Box::new(crate::bitbucket_workspace::source_from_params(
                params,
                http,
                limits,
                respect_default_excludes,
            ...
            Some(_) => Err(feature_unavailable("bitbucket-workspace", "bitbucket")),
        },
