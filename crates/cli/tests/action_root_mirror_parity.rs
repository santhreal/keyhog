...
//! inner file with only two source-root expressions rewritten for the shallower
...
//! in the other.
//!
//! The only delta is `ACTION_SOURCE_ROOT`: it is `github.action_path` at the
//! root (the checkout root itself) instead of `github.action_path/../../..`
//! from three levels deep. It appears twice, in the version-resolve step and
//! the source-build step. The resolved root crosses step boundaries and both
//! entrypoints invoke the same nested scan wrapper through it.
...
const BACKEND_INPUT_CONTRACT: &str = concat!(
    "      Scan backend for a checked-out tree. Published refs use the lean CI build\n",
    "      and accept auto or cpu. Portable branch and commit source refs require cpu."
);
...
fn root_action_is_the_locked_mirror_of_the_inner_action() {
    ...
    let expected_root = inner.replace(SOURCE_ROOT_FROM_INNER, SOURCE_ROOT_AT_ROOT);
    ...
    assert_eq!(
        ...
        "root action.yml has drifted from the inner action.yml mirror. Edit \
         ...
         regenerate the root by rewriting only the two source-root expressions \
         this test documents. Do not hand-edit one copy alone."
    );
}
