//! E2E R5-T-CLI: scan git history finds committed secret.rs.
...
#[test]
fn scan_git_history_finds_committed_secret() {
    ...
    std::process::Command::new("git").args(["add", "secret.env"]).current_dir(repo).status().expect("git add");
    std::process::Command::new("git").args(["commit", "-m", "leak", "-q"]).current_dir(repo).status().expect("git commit");
    let output = Command::new(binary())
        .args(["scan", "--no-daemon", "--git-history", "--format", "json", "--no-suppress-test-fixtures", "--max-commits", "10"])
    ...
    assert!(findings.as_array().unwrap().iter().any(|f| f["detector_id"].as_str() == Some("aws-access-key")));
}
