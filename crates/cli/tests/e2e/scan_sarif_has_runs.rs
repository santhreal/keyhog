...
fn scan_sarif_has_runs() {
    ...
    let output = Command::new(binary()).args(["scan", "--no-daemon", "--format", "sarif"]).arg(&path).output().expect("spawn");
    let stdout = String::from_utf8_lossy(&output.stdout);
    let sarif: serde_json::Value = serde_json::from_str(&stdout).expect("sarif json");
    assert!(sarif.get("runs").and_then(|v| v.as_array()).is_some_and(|a| !a.is_empty()), "sarif must have runs; got: {sarif}");
}
