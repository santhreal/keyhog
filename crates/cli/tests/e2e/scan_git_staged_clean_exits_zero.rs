//! E2E R5-T-CLI: scan git staged clean exits zero.rs.
...
#[test]
fn scan_git_staged_clean_exits_zero() {
    ...
    assert!(serde_json::from_str::<serde_json::Value>(&String::from_utf8_lossy(&output.stdout)).unwrap().as_array().unwrap().is_empty());
}
