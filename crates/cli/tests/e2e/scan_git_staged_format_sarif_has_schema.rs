//! E2E R5-T-CLI: scan git staged format sarif has schema.rs.
...
#[test]
fn scan_git_staged_format_sarif_has_schema() {
    ...
    let sarif: serde_json::Value = serde_json::from_str(&String::from_utf8_lossy(&output.stdout)).expect("sarif json");
    assert_eq!(sarif["$schema"].as_str(), Some("https://json.schemastore.org/sarif-2.1.0.json"));
}
