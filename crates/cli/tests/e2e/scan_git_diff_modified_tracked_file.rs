...
fn scan_git_diff_modified_tracked_file() {
    ...
    std::process::Command::new("git").args(["add", "tracked.env"]).current_dir(repo).status().expect("git add");
    std::process::Command::new("git").args(["commit", "-m", "init", "-q"]).current_dir(repo).status().expect("git commit");
    std::fs::write(repo.join("tracked.env"), "AWS_ACCESS_KEY_ID=AKIAKPQXRMSNTBVWYZBN\n").unwrap();
    let output = Command::new(binary())
        .args(["scan", "--no-daemon", "--git-diff", "HEAD", "--format", "json", "--no-suppress-test-fixtures"])
    ...
    let cited = findings.as_array().unwrap().iter().any(|f| {
        f["location"]["file_path"].as_str().map(|p| p.ends_with("tracked.env")).unwrap_or(false)
    });
