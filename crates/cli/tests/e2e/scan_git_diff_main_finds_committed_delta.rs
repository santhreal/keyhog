//! E2E R5-T-CLI: scan git diff main finds committed delta.rs.
...
#[test]
fn scan_git_diff_main_finds_committed_delta() {
    ...
    std::process::Command::new("git").args(["commit", "-m", "add secret", "-q"]).current_dir(repo).status().expect("git commit");
    let output = Command::new(binary())
        .args(["scan", "--no-daemon", "--git-diff", "HEAD~1", "--format", "json", "--no-suppress-test-fixtures"])
