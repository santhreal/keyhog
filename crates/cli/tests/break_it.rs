...
fn test_empty_parse_byte_size() {
    assert_eq!(API.parse_byte_size("").unwrap(), 0);
}

#[test]
fn test_empty_parse_decode_depth() {
    assert!(API.parse_decode_depth("").is_err());
}

#[test]
fn test_empty_parse_min_confidence() {
    assert!(API.parse_min_confidence("").is_err());
}

#[test]
fn test_empty_filter_inline_suppressions() {
    let result = API.filter_inline_suppressions(vec![]);
    assert!(result.is_empty());
}

#[test]
fn test_null_bytes_parse_byte_size() {
    assert!(API.parse_byte_size("\0").is_err());
    assert!(API.parse_byte_size("10\0MB").is_err());
}
...
fn test_null_bytes_parse_min_confidence() {
    assert!(API.parse_min_confidence("\0").is_err());
}

#[test]
fn test_null_bytes_filter_inline_suppressions() {
    let m = dummy_match(Some("test.txt\0"), Some(1), "test\0");
    let result = API.filter_inline_suppressions(vec![m.clone()]);
...
fn test_max_u64_parse_byte_size() {
    ...
    assert!(API.parse_byte_size(&max_u64_str).is_err());
}

#[test]
fn test_max_usize_parse_decode_depth() {
    let max_usize_str = format!("{}", usize::MAX);
    assert!(API.parse_decode_depth(&max_usize_str).is_err());
}

#[test]
fn test_large_f64_parse_min_confidence() {
    assert!(API.parse_min_confidence("1e100").is_err());
}
...
fn test_1mb_parse_byte_size() {
    ...
    assert!(API.parse_byte_size(&large_str).is_err());
}

#[test]
fn test_1mb_parse_decode_depth() {
    let large_str = "1".repeat(1024 * 1024);
    assert!(API.parse_decode_depth(&large_str).is_err());
}
...
fn test_1mb_parse_min_confidence() {
    ...
    let parsed = API
        .parse_min_confidence(&large_str)
        .expect("1MB 0.111… must parse");
...
fn test_1mb_filter_inline_suppressions() {
    ...
    let result = API.filter_inline_suppressions(vec![m]);
    assert_eq!(result.len(), 1);
}
...
fn test_malformed_parse_byte_size() {
    assert!(API.parse_byte_size("MB").is_err());
    assert!(API.parse_byte_size("10 M B").is_err());
    assert!(API.parse_byte_size("10XB").is_err());
}

#[test]
fn test_malformed_parse_decode_depth() {
    assert!(API.parse_decode_depth("5.5").is_err());
    assert!(API.parse_decode_depth("-1").is_err());
}

#[test]
fn test_malformed_parse_min_confidence() {
    assert!(API.parse_min_confidence("abc").is_err());
    assert!(API.parse_min_confidence("0.5.5").is_err());
}

#[test]
fn test_unicode_parse_byte_size() {
    assert!(API.parse_byte_size("10💩").is_err());
    assert!(API.parse_byte_size("١٠MB").is_err());
}

#[test]
fn test_unicode_parse_decode_depth() {
    assert!(API.parse_decode_depth("５").is_err()); // Fullwidth digit 5
}

#[test]
fn test_unicode_parse_min_confidence() {
    assert!(API.parse_min_confidence("０.５").is_err());
}
...
fn test_duplicate_filter_inline_suppressions() {
    ...
    let result = API.filter_inline_suppressions(vec![m1, m2]);
...
fn test_off_by_one_parse_decode_depth() {
    assert!(API.parse_decode_depth("0").is_err());
    assert!(API.parse_decode_depth("11").is_err());
}

#[test]
fn test_off_by_one_parse_min_confidence() {
    assert!(API.parse_min_confidence("-0.00001").is_err());
    assert!(API.parse_min_confidence("1.00001").is_err());
}
...
fn test_exhaustion_filter_inline_suppressions() {
    ...
    let result = API.filter_inline_suppressions(matches);
...
fn test_negative_byte_size() {
    assert!(API.parse_byte_size("-10MB").is_err());
}
...
fn test_byte_size_lowercase_unit() {
    ...
    // upper-case form. The previous assertion `API.parse_byte_size("10gb").is_ok()`
    // would still pass if the parser silently treated `gb` as bytes (10 bytes)
    // or as MB instead of GB; assert the actual value.
    let lower = API
        .parse_byte_size("10gb")
        .expect("lowercase gb must parse");
    let upper = API
        .parse_byte_size("10GB")
        .expect("uppercase GB must parse");
...
fn test_invalid_unit_byte_size() {
    assert!(API.parse_byte_size("10ZB").is_err());
}
