...
//!
//! The hidden coherence oracle `KEYHOG_PRINT_EFFECTIVE_CONFIG=1`
//! (`orchestrator_config::print_effective_config_if_requested`) is the
//! ground-truth surface for engine-config composition: it prints the
...
/// Run `keyhog <args...>` with the effective-config oracle env var set.
/// Returns (stdout, stderr, code). The oracle prints the resolved config and
/// exits SUCCESS before any source is scanned.
