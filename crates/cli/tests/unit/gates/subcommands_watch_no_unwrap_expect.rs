//! Gate `subcommands::watch`: no .unwrap( / .expect( in production source lines.

#[test]
fn subcommands_watch_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "subcommands::watch: unwrap/expect in production source at {:?}",
