//! Gate `subcommands::daemon`: no .unwrap( / .expect( in production source lines.

#[test]
fn subcommands_daemon_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "subcommands::daemon: unwrap/expect in production source at {:?}",
