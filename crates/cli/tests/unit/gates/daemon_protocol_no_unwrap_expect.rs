//! Gate `daemon::protocol`: no .unwrap( / .expect( in production source lines.

#[test]
fn daemon_protocol_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "daemon::protocol: unwrap/expect in production source at {:?}",
