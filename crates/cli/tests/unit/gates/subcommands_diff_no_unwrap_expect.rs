//! Gate `subcommands::diff`: no .unwrap( / .expect( in production source lines.

#[test]
fn subcommands_diff_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "subcommands::diff: unwrap/expect in production source at {:?}",
