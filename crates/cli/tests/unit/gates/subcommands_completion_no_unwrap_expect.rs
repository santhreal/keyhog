//! Gate `subcommands::completion`: no .unwrap( / .expect( in production source lines.

#[test]
fn subcommands_completion_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "subcommands::completion: unwrap/expect in production source at {:?}",
