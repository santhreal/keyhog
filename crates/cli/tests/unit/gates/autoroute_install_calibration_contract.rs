...
fn installer_primes_autoroute_and_runtime_requires_explicit_calibration() {
    ...
    assert!(
        backend.contains("binary_version")
            ...
            && backend.contains("AUTOROUTE_CACHE_VERSION: u32 = 34")
    ...
    assert!(
        orchestrator_mod.contains("cached_autoroute_router_for_default_config")
            ...
            && orchestrator_mod.contains("setup_default_scan_runtime")
            && watch.contains("setup_default_scan_runtime")
            ...
            && scan_system.contains("setup_default_scan_runtime")
