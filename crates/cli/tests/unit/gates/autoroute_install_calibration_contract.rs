...
fn installer_primes_autoroute_and_runtime_requires_explicit_calibration() {
    ...
    let scanner_megakernel_dispatch = std::fs::read_to_string(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../scanner/src/engine/megakernel_dispatch.rs"
    ))
    .expect("scanner megakernel dispatch source readable");
    ...
    assert!(
        scanner_engine.contains("gpu_degrade_count")
            && scanner_megakernel_dispatch.contains("gpu_degrade_count")
    ...
    assert!(
        dispatch.contains("autoroute_config_digest(&self.effective_config)")
            ...
            && dispatch.contains("keyhog_core::compute_spec_hash(&self.detectors)")
            && dispatch.contains("keyhog_core::hex_encode")
            && fused.contains("keyhog_core::compute_spec_hash(&self.detectors)")
            && fused.contains("keyhog_core::hex_encode")
    ...
    assert!(
        orchestrator_mod.contains("cached_autoroute_router_for_default_config")
            ...
            && watch.contains("cached_autoroute_router_for_default_config")
            && watch.contains("router.choose(")
            && watch.contains("scan_with_backend(&chunk, backend)")
