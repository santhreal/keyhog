//! Gate `subcommands::backend`: no .unwrap( / .expect( in production source lines.

#[test]
fn subcommands_backend_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "subcommands::backend: unwrap/expect in production source at {:?}",
