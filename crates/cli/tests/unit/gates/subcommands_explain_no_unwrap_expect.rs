//! Gate `subcommands::explain`: no .unwrap( / .expect( in production source lines.

#[test]
fn subcommands_explain_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "subcommands::explain: unwrap/expect in production source at {:?}",
