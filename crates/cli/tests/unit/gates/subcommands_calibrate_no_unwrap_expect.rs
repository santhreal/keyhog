//! Gate `subcommands::calibrate`: no .unwrap( / .expect( in production source lines.

#[test]
fn subcommands_calibrate_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "subcommands::calibrate: unwrap/expect in production source at {:?}",
