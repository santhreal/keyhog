//! Gate `test_fixture_suppressions`: no .unwrap( / .expect( in production source lines.

#[test]
fn test_fixture_suppressions_no_unwrap_expect() {
    let path = concat!(
    ...
    assert!(
        offenders.is_empty(),
        "test_fixture_suppressions: unwrap/expect in production source at {:?}",
