//! Gate `subcommands::detectors`: no .unwrap( / .expect( in production source lines.

#[test]
fn subcommands_detectors_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "subcommands::detectors: unwrap/expect in production source at {:?}",
