//! Gate `subcommands::scan_system`: no .unwrap( / .expect( in production source lines.

#[test]
fn subcommands_scan_system_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "subcommands::scan_system: unwrap/expect in production source at {:?}",
