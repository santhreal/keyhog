//! Gate `path_validation`: no .unwrap( / .expect( in production source lines.

#[test]
fn path_validation_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "path_validation: unwrap/expect in production source at {:?}",
