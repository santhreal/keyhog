//! Gate `subcommands::hook`: no .unwrap( / .expect( in production source lines.

#[test]
fn subcommands_hook_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "subcommands::hook: unwrap/expect in production source at {:?}",
