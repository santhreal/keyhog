//! Gate `subcommands::scan`: no .unwrap( / .expect( in production source lines.

#[test]
fn subcommands_scan_no_unwrap_expect() {
    ...
    assert!(
        offenders.is_empty(),
        "subcommands::scan: unwrap/expect in production source at {:?}",
