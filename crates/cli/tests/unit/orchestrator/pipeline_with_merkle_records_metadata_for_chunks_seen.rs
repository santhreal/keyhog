...
fn pipeline_with_merkle_records_metadata_for_chunks_seen() {
    ...
    // The read-free skip trusts a row only when the recorded inode change time
    // matches, so the fixture carries one and the assertion queries it back.
    chunk.metadata.ctime_ns = Some(1_700_000_000_100_000_000);
