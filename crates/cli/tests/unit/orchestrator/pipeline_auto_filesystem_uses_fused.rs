...
fn explicit_gpu_backends_keep_coalesced_batch_pipeline() {
    assert!(API.backend_requires_coalesced_batch_pipeline_for_test(Some(ScanBackend::GpuCuda)));
    assert!(API.backend_requires_coalesced_batch_pipeline_for_test(Some(ScanBackend::GpuWgpu)));
}
