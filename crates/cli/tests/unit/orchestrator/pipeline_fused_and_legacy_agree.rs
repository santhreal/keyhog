//! Pipeline parity: the fused parallel read+scan path and the legacy batch
...
#[test]
fn fused_and_legacy_pipelines_agree_on_filesystem_scan() {
    let dir = planted_dir();
    let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());

    // Pin the deterministic SIMD backend so the fused path engages regardless
    // of whether the test host has a GPU (a real GPU would otherwise route the
    // default no-override scan to the batch pipeline).
    std::env::set_var("KEYHOG_BACKEND", "simd");

    std::env::remove_var("KEYHOG_LEGACY_PIPELINE");
    let fused = scan_findings(dir.path());

    std::env::set_var("KEYHOG_LEGACY_PIPELINE", "1");
    let legacy = scan_findings(dir.path());
    ...
    assert_eq!(
        fused, legacy,
        "fused and legacy pipelines must produce the identical distinct finding set"
    );
