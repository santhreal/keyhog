...
fn write_bytes_atomic(path: &Path, bytes: &[u8]) -> std::io::Result<()> {
    // LAW10: no runtime effect: relative path without parent uses current directory
    let parent = path.parent().unwrap_or_else(|| Path::new("."));
    let tmp: PathBuf = parent.join(format!(
        ".{}.tmp",
        path.file_name()
            .and_then(|n| n.to_str())
            .unwrap_or("artifact") // LAW10: reporting-only: fallback label for unnamed temp file
    ));
    {
