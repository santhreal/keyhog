...
async fn dispatch(state: &ServerState, request: Request) -> Response {
    ...
    match request {
        ...
        Request::GuardCommitBegin {
            ...
            let canonical_bytes = canonical_repo.to_string_lossy();
            let identity = match state
                .guard
                .root_policy_identity(canonical_bytes.as_bytes())
        ...
        Request::GuardCommitFinish {
            ...
            let commit_bytes = commit_root.to_string_lossy();
            let policy_identity = state
                .guard
                .root_policy_identity(commit_bytes.as_bytes())
                .or_else(|| state.guard.policy_identity())
                .unwrap_or_else(|| {
