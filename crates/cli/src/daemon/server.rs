...
async fn dispatch(state: &ServerState, request: Request) -> Response {
    ...
    match request {
        ...
        Request::GuardCommitBegin {
            ...
            let canonical_root = match std::fs::canonicalize(&repo_path) {
                Ok(p) => p,
                Err(_) => std::path::PathBuf::from(&repo_path),
            };
            let canonical_bytes =
                std::os::unix::ffi::OsStrExt::as_bytes(canonical_root.as_os_str());
            // Get the policy identity for attestation lookup.
            let identity = match state
                .guard
                .root_policy_identity(canonical_bytes)
                .or_else(|| state.guard.policy_identity())
                .or_else(|| {
                    Some(compute_root_policy_identity(
                        &canonical_root,
                        KEYHOG_VERSION,
        ...
        Request::GuardCommitFinish {
            ...
            let commit_bytes = std::os::unix::ffi::OsStrExt::as_bytes(commit_root.as_os_str());
            let policy_identity = state
                .guard
                .root_policy_identity(commit_bytes)
                .or_else(|| state.guard.policy_identity())
                .unwrap_or_else(|| {
        ...
        Request::GuardStatus { root } => match state.guard.root_record(root.as_bytes()) {
            Some(record) => {
                ...
                let root_policy = state
                    .guard
                    .root_policy_identity(root.as_bytes())
                    .or_else(|| state.guard.policy_identity());
                let build_identity_short = root_policy
                    .as_ref()
                    .and_then(|id| id.short_digest().ok())
                    .unwrap_or_default();
                let detector_digest_short = root_policy
                    .as_ref()
                    .map(|id| {
                        id.detector_digest
                            .get(..12)
                    ...
                    .unwrap_or_default();
                let suppression_digest_short = root_policy
                    .as_ref()
                    .map(|id| {
                    ...
                    .unwrap_or_default();
                let config_digest_short = root_policy
                    .as_ref()
                    .map(|id| {
                        id.config_digest
                            .get(..12)
