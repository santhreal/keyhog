...
pub(crate) fn filter_disabled_detectors(
    ...
    filter_disabled_detectors_collect(detectors, disabled_detectors, &mut removed)
}

pub(crate) fn filter_disabled_detectors_collect(
    detectors: &mut Vec<DetectorSpec>,
    disabled_detectors: &std::collections::HashSet<String>,
    all_removed: &mut std::collections::HashSet<String>,
) -> usize {
...
fn setup_default_scan_runtime_with_rayon_policy(
    ...
    if !disabled_detectors.is_empty() {
        let before = detectors.len();
        filter_disabled_detectors_collect(
            &mut detectors,
            &effective_config.disabled_detectors,
            &mut disabled_detectors,
        );
...
impl ScanOrchestrator {
    ...
    pub(crate) fn new(mut args: ScanArgs) -> Result<Self> {
        ...
        if !disabled_detectors.is_empty() {
            let before = detectors.len();
            let dropped = filter_disabled_detectors_collect(
                &mut detectors,
                &effective_config.disabled_detectors,
                &mut disabled_detectors,
            );
