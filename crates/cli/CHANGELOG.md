...
- test(cli): the empty-directory exit contract is wired into the e2e manifest, and daemon lifecycle tests serialize real daemon starts with an explicit CPU diagnostic route instead of depending on missing autoroute calibration or competing detector compiles.
