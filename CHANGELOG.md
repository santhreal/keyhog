...
- Authenticated execution-pack pages use random-access mmap advice and are discarded before matcher hydration; the selected mapping is released after construction. Normal scans fault back only the selected sections while decoding them into owned runtime state, so the complete pack mapping no longer overlaps the resident scanner.
