...
- Findings now carry a canonical evidence verdict with exact `review`, `likely`, or `confirmed` tier and reason code. Public reports use optional `evidence_score` instead of `confidence`; JSON and JSONL report schema 2, baseline schema 2, and daemon wire 15 reject stale records that lack evidence. Deduplication retains the strongest proof independently from the scanner's internal score, and live verification upgrades the verdict to `confirmed/live-verification`.
...
- Emitted candidates in Markdown, roff/man pages, shell scripts, Dockerfiles, and Containerfiles receive bounded source roles for prose, inline code, shell-fenced commands, option declarations, environment assignments, and command argument values. Structured detector and rule fields derive regex-definition, test-fixture, and prose roles from validated Tier-B markers. Malformed, truncated, unsupported, and over-budget input abstains without changing recall or public `RawMatch` output.
...
- The redacted real-repository quality gate validates each persisted identity receipt against the freshly captured candidate binary and measures unlabeled noise per MLOC. Labeled findings and deterministic canaries contribute to recall without inflating the noise density.
...
- `keyhog triage` imports only the current versioned redacted finding envelope and emits separately versioned scoped runtime-suppression and pattern-training artifacts. Records require the 16-hex active detector digest plus authoritative scanner pattern indexes, candidate channels, source roles, and context classes; typed mutually exclusive scopes, closed reasons, input and record limits, and non-symlink create-new outputs prevent credential, context, path, and stale-policy persistence. Any detector corpus change invalidates every persisted triage artifact. Pattern-feedback-only decisions cannot produce runtime suppressions.
