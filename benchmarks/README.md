...
`make readme-matrix` owns the KeyHog accuracy, configuration, and daemon
panels. Never edit generated rows by hand.
...
make readme-matrix # measure and inject the README accuracy/configuration/daemon panels
make readme-matrix-check # verify snapshot, reports, and README generated bytes
...
and daemon row. It writes the provenance-bound
`reports/readme-matrix.json` snapshot, renders `accuracy-matrix.md`,
`configuration-matrix.md`, and `daemon-matrix.md`, then injects the three README
panels. A missing route, unavailable backend, stale binary, mixed host, changed
daemon corpus byte, or omitted source-state classification fails capture. The
release target requires one scanner version, executable digest, detector
digest, host identity, and clean source classification across all selected
rows.

For a local comparison that intentionally measures an uncommitted scanner,
label it rather than presenting it as release evidence:

```bash
KEYHOG_BENCH_ALLOW_DIRTY=1 \
  make readme-matrix README_MATRIX_SOURCE_STATE=developer-dirty
```
