...
def to_toml_overlay(recs: list[Recommendation]) -> str:
    ...
    lines = [
        ...
        "# benchmark corpus (each floor cuts ≥1 FP, loses 0 TP). Apply each",
        "# value to `min_confidence` under [detector] in detectors/<id>.toml,",
        "# then rebuild + re-score to confirm. Generated by `bench calibrate`.",
        "",
    ]
    for r in wins:
        lines.append(
            f"# {r.detector_id}: cuts {r.lossless_fp_cut} FP "
            f"(was P={r.current_precision:.3f}, tp={r.tp}, fp={r.fp})"
        )
        lines.append(f'["{r.detector_id}"]')
