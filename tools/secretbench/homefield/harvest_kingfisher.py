...
on exactly these strings. Harvesting them lets the existing scorer
(`scoring/score.py`) run keyhog and the three competitors over
kingfisher's home turf and answer the only fair question: how close does
keyhog get to kingfisher on kingfisher's own truth, and which services
does kingfisher cover that keyhog misses (a capability gap).

Output (drop-in for score.py):
    homefield/kingfisher/corpus/manifest.jsonl
    homefield/kingfisher/corpus/<shard>/<id>.txt
"""
...
OUT = pathlib.Path(__file__).resolve().parent / "kingfisher" / "corpus"
