...
corpus so the existing scorer (`scoring/score.py`) can run keyhog and the
three competitors over betterleaks' home turf unchanged.
...
Output (drop-in for score.py):
    homefield/betterleaks/corpus/manifest.jsonl
    homefield/betterleaks/corpus/<shard>/<id>.txt
"""
...
OUT = pathlib.Path(__file__).resolve().parent / "betterleaks" / "corpus"
