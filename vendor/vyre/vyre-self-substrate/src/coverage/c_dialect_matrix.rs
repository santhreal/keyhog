...
pub fn validate_c_dialect_target_manifest(
    ...
    for (evidence, needle) in [
        ...
        (
            "zero silent CPU fallbacks",
            "zero_silent_cpu_fallbacks = true",
        ),
...
mod tests {
    ...
    fn dialect_matrix_accepts_all_phases_and_lower_step_scope() {
        let proof =
            validate_c_dialect_matrix(&rows()).expect("complete dialect matrix should pass");
    ...
    fn dialect_matrix_accepts_committed_linux_math_target_manifest() {
        ...
        .expect("committed Linux lib/math target manifest should pass");
