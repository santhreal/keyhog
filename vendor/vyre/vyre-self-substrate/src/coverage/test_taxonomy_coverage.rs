    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::SuiteArtifactThresholdMiss {
                ...
                f,
                "test taxonomy suite artifact `{suite}` {field}={observed} missed required {required}. Fix: restore Vyre/Weir/Vyrec test coverage for that taxonomy."
            ),
        }
    }
}
...
pub fn validate_test_taxonomy_suite_artifacts(
    ...
    for (suite, artifact, min_total, min_vyre, min_weir, min_vyrec) in suites {
        ...
        let weir_file_count = suite_number_field(suite, artifact, "weir_file_count")?;
        ...
        suite_at_least(suite, "weir_file_count", weir_file_count, *min_weir)?;
...
mod tests {
    ...
    fn taxonomy_coverage_accepts_all_required_test_classes() {
        let proof = validate_test_taxonomy_coverage(&records("cuda-megakernel"))
            .expect("complete test taxonomy should pass");
    ...
    fn taxonomy_coverage_accepts_committed_suite_artifacts() {
        let proof = validate_test_taxonomy_suite_artifacts(&suite_artifacts())
            .expect("committed taxonomy suite artifacts should pass");
