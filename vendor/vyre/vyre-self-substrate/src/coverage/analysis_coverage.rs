...
pub struct WeirAnalysisCoverageRecord<'a> {
    /// Analysis.
    pub analysis: WeirAnalysis,
    /// Evidence kind.
    pub evidence_kind: WeirAnalysisEvidence,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct WeirAnalysisCoverageProof {
    /// Analysis count.
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct WeirAnalysisArtifactProof {
...
pub enum WeirAnalysisCoverageError {
    /// Records are empty.
    EmptyRecords,
    /// Metadata is empty.
    EmptyMetadata {
        /// Analysis.
        analysis: WeirAnalysis,
        /// Field.
        field: &'static str,
    },
    ...
    MissingEvidence {
        ...
        analysis: WeirAnalysis,
        /// Evidence kind.
        evidence_kind: WeirAnalysisEvidence,
    },
...
impl std::fmt::Display for WeirAnalysisCoverageError {
...
impl std::error::Error for WeirAnalysisCoverageError {}

const REQUIRED_ANALYSES: &[WeirAnalysis] = &[
    WeirAnalysis::ReachingDefinitions,
    WeirAnalysis::Liveness,
    WeirAnalysis::PointsTo,
    WeirAnalysis::Slicing,
    WeirAnalysis::Ifds,
];

const REQUIRED_EVIDENCE: &[WeirAnalysisEvidence] = &[
    WeirAnalysisEvidence::CallerOwnedScratch,
    WeirAnalysisEvidence::Residency,
    WeirAnalysisEvidence::MonotonicityConvergence,
    WeirAnalysisEvidence::LatticeLaws,
    WeirAnalysisEvidence::AdversarialGraphs,
    WeirAnalysisEvidence::GpuAllocationEvidence,
];

/// Validate Dataflow analysis release coverage.
pub fn validate_analysis_coverage(
    records: &[WeirAnalysisCoverageRecord<'_>],
) -> Result<WeirAnalysisCoverageProof, WeirAnalysisCoverageError> {
    if records.is_empty() {
        return Err(WeirAnalysisCoverageError::EmptyRecords);
    }
    let mut by_analysis: BTreeMap<WeirAnalysis, BTreeSet<WeirAnalysisEvidence>> = BTreeMap::new();
    for record in records {
        for (field, value) in [("command", record.command), ("evidence", record.evidence)] {
            if value.trim().is_empty() {
                return Err(WeirAnalysisCoverageError::EmptyMetadata {
        ...
        if !record.command.trim_start().starts_with("./cargo_full ") {
            return Err(WeirAnalysisCoverageError::CommandDoesNotUseCargoFull {
                analysis: record.analysis,
    ...
    for analysis in REQUIRED_ANALYSES {
        let Some(evidence) = by_analysis.get(analysis) else {
            return Err(WeirAnalysisCoverageError::MissingAnalysis {
                analysis: *analysis,
            });
        };
        for evidence_kind in REQUIRED_EVIDENCE {
            if !evidence.contains(evidence_kind) {
                return Err(WeirAnalysisCoverageError::MissingEvidence {
                    analysis: *analysis,
...
) -> Result<WeirAnalysisArtifactProof, WeirAnalysisCoverageError> {
...
fn artifact_contains(
    ...
    evidence: &'static str,
    needle: &str,
) -> Result<(), WeirAnalysisCoverageError> {
    if artifact.contains(needle) {
        Ok(())
    } else {
        Err(WeirAnalysisCoverageError::ArtifactMissingEvidence { evidence })
    }
}
...
fn require_at_least(
    ...
    if observed >= required {
        ...
        Err(WeirAnalysisCoverageError::ArtifactThresholdMiss {
            field,
            observed,
            required,
        })
...
fn number_field(artifact: &str, field: &'static str) -> Result<u64, WeirAnalysisCoverageError> {
    let key = format!("\"{field}\"");
    let start = artifact
        .find(&key)
        .ok_or(WeirAnalysisCoverageError::ArtifactMissingNumber { field })?;
    ...
    let digits = after_colon
        ...
        .collect::<String>();
    if digits.is_empty() {
        return Err(WeirAnalysisCoverageError::ArtifactMissingNumber { field });
    }
    digits
        .parse::<u64>()
        .map_err(|_| WeirAnalysisCoverageError::ArtifactMissingNumber { field })
}
...
mod tests {
    ...
    fn analysis_coverage_accepts_all_required_evidence() {
        let proof = validate_analysis_coverage(&records())
            .expect("complete Dataflow analysis coverage should pass");
    ...
    fn analysis_coverage_rejects_missing_ifds() {
        let records: Vec<_> = records()
            .into_iter()
            .filter(|record| record.analysis != WeirAnalysis::Ifds)
            .collect();

        assert_eq!(
            validate_analysis_coverage(&records).expect_err("missing IFDS should fail"),
            WeirAnalysisCoverageError::MissingAnalysis {
                analysis: WeirAnalysis::Ifds,
            }
        );
    }
    ...
    fn analysis_coverage_rejects_missing_evidence_and_raw_cargo() {
        ...
        missing_evidence_records.retain(|record| {
            !(record.analysis == WeirAnalysis::Liveness
                && record.evidence_kind == WeirAnalysisEvidence::GpuAllocationEvidence)
        });
        assert_eq!(
            validate_analysis_coverage(&missing_evidence_records)
                .expect_err("missing evidence should fail"),
            WeirAnalysisCoverageError::MissingEvidence {
                analysis: WeirAnalysis::Liveness,
                evidence_kind: WeirAnalysisEvidence::GpuAllocationEvidence,
            }
        );
        ...
        assert_eq!(
            ...
            WeirAnalysisCoverageError::CommandDoesNotUseCargoFull {
                analysis: WeirAnalysis::ReachingDefinitions,
    ...
    fn analysis_coverage_accepts_committed_api_and_integration_artifacts() {
        let proof = validate_committed_analysis_artifacts(
            include_str!("../../../release/evidence/weir/weir-analysis-api-matrix.json"),
            include_str!("../../../release/evidence/weir/weir-vyre-integration-tests.json"),
        )
        .expect("committed Dataflow analysis artifacts should pass");
    ...
    fn analysis_coverage_rejects_partial_committed_artifacts() {
        ...
        assert_eq!(
            err,
            WeirAnalysisCoverageError::ArtifactMissingEvidence {
                evidence: "reaching API",
            }
        );
    }

    fn records() -> Vec<WeirAnalysisCoverageRecord<'static>> {
        let mut records = Vec::new();
        for analysis in REQUIRED_ANALYSES {
            for evidence_kind in REQUIRED_EVIDENCE {
                records.push(WeirAnalysisCoverageRecord {
                    analysis: *analysis,
                    evidence_kind: *evidence_kind,
                    command: "./cargo_full test -j1 -p dataflow",
                    evidence: "release/weir/analysis-coverage.md",
                });
            }
        }
        records
