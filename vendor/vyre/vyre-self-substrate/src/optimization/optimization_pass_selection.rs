...
use crate::optimization_registry::{OptimizationPassExplanation, OptimizationRegistry};
...
mod tests {
    ...
    fn selector_keeps_tiny_one_shot_workloads_empty() {
        ...
        .expect("valid tiny workload should select");
    ...
    fn selector_enables_cuda_sparse_repeated_hot_path() {
        ...
        .expect("valid sparse workload should select");
        ...
        registry
            ...
            .expect("selected sparse hot-path passes must satisfy registry order");
    }
    ...
    fn selector_reuses_caller_owned_pass_vector_and_validates_order() {
        ...
        .expect("valid sparse workload should select into caller storage");
        ...
        registry
            ...
            .expect("selected pass order must already satisfy registry order");
    }
    ...
    fn selector_enables_dense_barrier_planning_without_sparse_queue() {
        ...
        .expect("valid dense workload should select");
        let ids: Vec<&str> = selected.iter().map(|entry| entry.pass.id).collect();
