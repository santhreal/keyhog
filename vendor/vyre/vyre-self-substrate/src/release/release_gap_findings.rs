    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::ArtifactThresholdMiss {
                ...
                f,
                "release gap suite artifact {field}={observed} missed required {required}. Fix: add real gap tests across Vyre, Weir, and Vyrec or close the approved beta gap explicitly."
            ),
        }
    }
}
...
pub fn validate_release_gap_suite_artifact(
    ...
    let weir_file_count = artifact_number_field(artifact, "weir_file_count")?;
    ...
    artifact_at_least("weir_file_count", weir_file_count, 1)?;
...
mod tests {
    ...
    fn gap_findings_accept_active_cargo_full_reproductions() {
        ...
        .expect("valid gap findings should pass");
    ...
    fn gap_findings_accept_committed_gap_suite_artifact() {
        ...
        .expect("committed gap suite artifact should pass");

        assert!(proof.file_count >= 20);
        assert!(proof.vyre_file_count >= 20);
        assert!(proof.weir_file_count >= 1);
