...
impl std::fmt::Display for ReleaseScopeDocError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MissingStableWeirSection { path } => write!(
                f,
            ...
            Self::ArtifactMissingEvidence { evidence } => write!(
                f,
                "release scope artifact is missing {evidence}. Fix: docs must separate stable Vyre, stable Weir, Vyrec beta lower-step scope, and parser/semantic evidence."
            ),
        }
    }
}
...
pub fn validate_release_scope_docs(
    ...
    for doc in docs {
        ...
        if !doc.stable_section {
            return Err(ReleaseScopeDocError::MissingStableWeirSection {
                path: doc.path.to_owned(),
            });
        }
...
pub fn validate_committed_release_scope_artifacts(
    ...
    for (artifact, evidence, needle) in [
        ...
        (readme_proof, "Dataflow consumer 0.0.1 API surface", "0.0.1"),
        (
            readme_proof,
...
mod tests {
    ...
    fn release_scope_docs_accept_stable_and_beta_separation() {
        let proof =
            validate_release_scope_docs(&[doc()]).expect("valid release scope doc should pass");
    ...
    fn release_scope_docs_accept_committed_scope_artifacts() {
        let proof = validate_committed_release_scope_artifacts(
            ...
            include_str!("../../../release/evidence/docs/weir-readme-proof.md"),
            include_str!("../../../release/evidence/docs/parser-doc-proof.md"),
            include_str!("../../../release/evidence/docs/c-parser-linux-proof.md"),
            include_str!("../../../release/evidence/docs/release-notes.md"),
        )
        .expect("committed release scope docs should preserve stable/beta boundaries");
