//! Release validation command matrix for Vyre, Weir, and Vyrec.
...
pub const RELEASE_VALIDATION_MATRIX: &[ReleaseValidationCommand] = &[
    ...
    ReleaseValidationCommand {
        ...
        working_dir: ".",
        requires_gpu_probe: false,
        evidence: "Vyre, Weir, and Vyrec duties have exactly one declared owner for parsing, graph formation, lowering, scheduling, dispatch, and validation",
    },
    ReleaseValidationCommand {
    ...
    ReleaseValidationCommand {
        ...
        area: "dataflow",
        command: "./cargo_full test -j1 --manifest-path ../../../dataflow/weir/Cargo.toml --features cpu-parity fixed_point_resident",
        working_dir: ".",
        requires_gpu_probe: true,
    ...
    ReleaseValidationCommand {
        id: "ifds-resident",
        area: "dataflow",
        command: "./cargo_full test -j1 --manifest-path ../../../dataflow/weir/Cargo.toml --features cpu-parity cuda_vyre_resident_ifds",
        working_dir: ".",
        requires_gpu_probe: true,
    ...
    ReleaseValidationCommand {
        ...
        area: "dataflow",
        command: "./cargo_full test -j1 --manifest-path ../../../dataflow/weir/Cargo.toml --features cpu-parity --test df_adversarial_oracles",
        working_dir: ".",
        requires_gpu_probe: false,
        evidence: "Dataflow consumer adversarial graph families cover empty, single-node, fan-in, fan-out, cycles, disconnected, and degenerate inputs",
    },
    ReleaseValidationCommand {
        id: "fuzz-bitset-oracles",
        area: "dataflow",
        command: "./cargo_full test -j1 --manifest-path ../../../dataflow/weir/Cargo.toml --features cpu-parity --test df_fuzz_bitset_oracles",
        working_dir: ".",
        requires_gpu_probe: false,
        evidence: "Dataflow consumer bitset oracle fuzzing covers arbitrary dataflow fact shapes under explicit parity feature",
    },
    ReleaseValidationCommand {
        id: "gap-bitset-oracle-edges",
        area: "dataflow",
        command: "./cargo_full test -j1 --manifest-path ../../../dataflow/weir/Cargo.toml --features cpu-parity --test df_gap_bitset_oracle_edges",
        working_dir: ".",
        requires_gpu_probe: false,
        evidence: "Dataflow consumer tracked gap tests make missing edge cases reproducible instead of silently green",
    },
    ReleaseValidationCommand {
        id: "property-points-to",
        area: "dataflow",
        command: "./cargo_full test -j1 --manifest-path ../../../dataflow/weir/Cargo.toml --features cpu-parity --test df_property_points_to",
        working_dir: ".",
        requires_gpu_probe: false,
    ...
    ReleaseValidationCommand {
        id: "property-ifds",
        area: "dataflow",
        command: "./cargo_full test -j1 --manifest-path ../../../dataflow/weir/Cargo.toml --features cpu-parity --test df_property_ifds",
        working_dir: ".",
        requires_gpu_probe: false,
    ...
    ReleaseValidationCommand {
        id: "property-slice",
        area: "dataflow",
        command: "./cargo_full test -j1 --manifest-path ../../../dataflow/weir/Cargo.toml --features cpu-parity --test df_property_slice_construction",
        working_dir: ".",
        requires_gpu_probe: false,
    ...
    ReleaseValidationCommand {
        ...
        area: "dataflow",
        command: "./cargo_full test -j1 --manifest-path ../../../dataflow/weir/Cargo.toml --features cpu-parity --test df_property_reaching_def_escapes",
        working_dir: ".",
        requires_gpu_probe: false,
    ...
    ReleaseValidationCommand {
        ...
        area: "dataflow",
        command: "./cargo_full test -j1 --manifest-path ../../../dataflow/weir/Cargo.toml --features cpu-parity --test df_parity_exact_primitives",
        working_dir: ".",
        requires_gpu_probe: true,
        evidence: "Dataflow consumer exact primitive parity runs on GPU against explicit parity-only oracles",
    },
    ReleaseValidationCommand {
        id: "scale-oracle-no-oom",
        area: "dataflow",
        command: "./cargo_full test -j1 --manifest-path ../../../dataflow/weir/Cargo.toml --features cpu-parity --test df_scale_oracle_no_oom",
        working_dir: ".",
        requires_gpu_probe: false,
        evidence: "Dataflow consumer scale and mass-fan inputs remain bounded and do not OOM",
    },
    ReleaseValidationCommand {
        id: "resident-fixed-point-benchmark",
        area: "dataflow",
        command: "./cargo_full bench -j1 --manifest-path ../../../dataflow/weir/Cargo.toml --features cpu-parity --bench resident_fixed_point_hot_path",
        working_dir: ".",
        requires_gpu_probe: true,
    ...
    ReleaseValidationCommand {
        ...
        area: "dataflow",
        command: "./cargo_full bench -j1 --manifest-path ../../../dataflow/weir/Cargo.toml --features cpu-parity --bench ifds_direct_resident_hot_path",
        working_dir: ".",
        requires_gpu_probe: true,
    ...
    ReleaseValidationCommand {
        ...
        working_dir: ".",
        requires_gpu_probe: true,
        evidence: "Vyrec and Dataflow consumer cannot disable required CUDA optimizations",
    },
    ReleaseValidationCommand {
...
fn validate_manifest_path(command: ReleaseValidationCommand) -> Result<(), String> {
    ...
    for (index, token) in tokens.iter().enumerate() {
        ...
        let full_path = vyre_workspace_root().join(command.working_dir).join(path);
...
fn manifest_path_for_command(
    ...
    if let Some(index) = tokens.iter().position(|token| *token == "--manifest-path") {
        ...
        return Ok(vyre_workspace_root().join(command.working_dir).join(path));
    }
...
fn source_root_for_command(
    ...
    if let Some(index) = tokens.iter().position(|token| *token == "--manifest-path") {
        ...
        let manifest = vyre_workspace_root().join(command.working_dir).join(path);
        return Ok(manifest
            .parent()
            .ok_or_else(|| {
                format!(
...
fn vyre_workspace_root() -> &'static std::path::Path {
    std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("vyre-self-substrate must live under the Vyre workspace root")
}
...
mod tests {
    ...
    fn release_matrix_uses_gpu_probe_and_cargo_full() {
        validate_release_validation_matrix().expect("release matrix must be valid");
    }
    ...
    fn release_matrix_requires_gpu_probe_for_every_cuda_gate() {
        require_cuda_gates_probe_gpu().expect("every CUDA release gate must require GPU probe");
    }

    #[test]
    fn release_matrix_has_unique_gate_ids() {
        require_unique_ids().expect("release matrix gate ids must be unique");
    }

    #[test]
    fn release_matrix_execution_plan_inserts_gpu_probe_before_required_gates() {
        let plan = release_validation_execution_plan().expect("release execution plan");
        for gate in RELEASE_VALIDATION_MATRIX
            .iter()
    ...
    #[test]
    fn release_matrix_covers_weir_and_semantic_coverage_gates() {
        for required in [
            "adversarial-oracles",
            "fuzz-bitset-oracles",
    ...
    fn release_matrix_rejects_missing_benchmark_target() {
        let err = validate_command(ReleaseValidationCommand {
            ...
            area: "dataflow",
            command: "./cargo_full bench -j1 --manifest-path ../../../dataflow/weir/Cargo.toml --bench definitely_missing_benchmark",
            working_dir: ".",
            requires_gpu_probe: true,
