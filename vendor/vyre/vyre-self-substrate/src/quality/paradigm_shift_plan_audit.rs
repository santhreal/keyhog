...
fn validate_plan_identity(plan_text: &str) -> Result<(), ParadigmShiftPlanAuditError> {
    for (field, needle) in [
        (
            "active plan title",
            "# Vyre / Dataflow consumer / Vyrec Paradigm-Shift Launch Plan",
        ),
        (
            "Vyre/Weir/Vyrec scope",
            "Scope: Vyre, Weir, and Vyrec only.",
        ),
        (
...
mod tests {
    ...
    fn committed_plan_maps_all_100_items_to_release_gates() {
        ...
        .expect("active 100-item plan should map to release validation gates");
