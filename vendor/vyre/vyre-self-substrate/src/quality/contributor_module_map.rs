...
pub enum ContributorDuty {
    ...
    Kernel,
    /// Dataflow consumer dataflow analyses.
    Analysis,
    /// Vyrec parser or preprocessing phases.
    ParserPhase,
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::ArtifactMissingEvidence { evidence } => write!(
                f,
                "contributor modularization artifact is missing {evidence}. Fix: keep Vyre, Weir, and Vyrec organized into fixtures, contracts, properties, backend, corpus, bench, and regression module families."
            ),
...
pub fn validate_committed_contributor_modularization_artifact(
    ...
    for (artifact, evidence, needle) in [
        ...
        (modularization_map, "Dataflow consumer surface", "\"surface\": \"dataflow\""),
        (modularization_map, "Vyrec surface", "\"surface\": \"vyrec\""),
        ...
        (test_architecture_doc, "Vyre Dataflow consumer Vyrec coverage", "Vyre, Weir, and the root `tools/vyrec` parser CLI surface"),
    ...
    for surface in ["vyre", "dataflow", "vyrec"] {
        for layer in [
            "fixtures",
            "contracts",
            "properties",
...
mod tests {
    ...
    fn contributor_module_map_accepts_all_required_duties() {
        let proof = validate_contributor_module_map(&entries())
            .expect("complete contributor module map should pass");
    ...
    fn contributor_module_map_accepts_committed_modularization_artifacts() {
        ...
        .expect("committed modularization artifacts should pass");
    ...
    fn contributor_module_map_rejects_partial_modularization_artifact() {
        ...
        assert_eq!(
            ...
            ContributorModuleMapError::ArtifactMissingEvidence {
                evidence: "Dataflow consumer surface",
            }
        );
    }
    ...
    fn entries() -> Vec<ContributorModuleMapEntry<'static>> {
        vec![
            ...
            entry(ContributorDuty::Analysis, "dataflow", "dataflow::analyses"),
            entry(ContributorDuty::ParserPhase, "vyrec", "vyrec::parser"),
            entry(
                ContributorDuty::Diagnostic,
                "vyre-self-substrate",
