//! Architecture boundary-map validation for Vyre, Weir, and Vyrec.
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::ArtifactMissingEvidence { evidence } => write!(
                f,
                "architecture boundary artifact is missing {evidence}. Fix: publish committed ownership evidence for parser components, Dataflow consumer dataflow, CUDA dispatch, and contributor topology."
            ),
...
/// Validate committed architecture-boundary evidence across parser, Weir, CUDA, and tests.
pub fn validate_committed_architecture_boundary_artifacts(
    ...
    for (artifact, evidence, needle) in [
        ...
        (
            distributed_parser_map,
            "Dataflow consumer dataflow parser owner",
            "\"id\": \"dataflow\"",
        ),
        (
            distributed_parser_map,
        ...
        (
            contracts,
            "Dataflow consumer alias/reaching/callgraph contract",
            "\"alias\"",
        ),
        (
            contracts,
        ...
        (
            contracts,
            "Dataflow consumer callgraph contract",
            "\"callgraph\"",
        ),
        (
            consumer_contracts,
        ...
        (
            analysis_matrix,
            "Dataflow consumer SSA analysis owner",
            "\"id\": \"ssa\"",
        ),
        (
            analysis_matrix,
            "Dataflow consumer points-to analysis owner",
            "\"id\": \"points_to\"",
        ),
        (
            analysis_matrix,
            "Dataflow consumer IFDS analysis owner",
            "\"id\": \"ifds\"",
        ),
        (
            analysis_matrix,
            "Dataflow consumer callgraph analysis owner",
            "\"id\": \"callgraph\"",
        ),
        (
            analysis_matrix,
            "Dataflow consumer liveness analysis owner",
            "\"id\": \"live\"",
        ),
        (
            analysis_matrix,
            "Dataflow consumer slice analysis owner",
            "\"id\": \"slice\"",
        ),
        (
            analysis_matrix,
            "no missing Dataflow consumer APIs",
            "\"missing_api_items\": []",
        ),
        (
            analysis_matrix,
            "no unresolved Dataflow consumer markers",
            "\"unresolved_markers\": []",
        ),
        (
            modularization_map,
        ...
        (
            modularization_map,
            "Dataflow consumer contributor topology",
            "\"surface\": \"dataflow\"",
        ),
        (
            modularization_map,
            "Vyrec contributor topology",
            "\"surface\": \"vyrec\"",
        ),
        (
            modularization_map,
...
mod tests {
    ...
    fn boundary_map_accepts_single_owner_per_required_duty() {
        let proof = validate_architecture_boundary_map(&boundaries())
            .expect("complete boundary map should pass");
    ...
    fn boundary_artifacts_accept_committed_architecture_evidence() {
        let proof = committed_architecture_artifact_proof()
            .expect("committed architecture evidence should prove release boundaries");
    ...
    fn boundary_artifacts_reject_missing_cuda_release_path() {
        let backend_matrix = include_str!("../../../release/evidence/backends/backend-matrix.json")
            .replace(
                "\"preferred_backend_id\": \"cuda\"",
        ...
        assert_eq!(
            validate_committed_architecture_boundary_artifacts(
                ...
                include_str!("../../../release/evidence/parser/weir-contracts.json"),
    ...
    #[test]
    fn boundary_artifacts_reject_missing_weir_api_ownership() {
        let analysis = include_str!("../../../release/evidence/weir/weir-analysis-api-matrix.json")
        ...
        assert_eq!(
            ...
            ArchitectureBoundaryMapError::ArtifactMissingEvidence {
                evidence: "Dataflow consumer points-to analysis owner",
            }
        );
    }
