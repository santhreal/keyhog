//! CUDA multi-query execution planning over one resident graph.
//!
...
pub struct CudaMultiQueryGroup {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CudaMultiQueryExecutionPlan {
    /// Groups in deterministic graph/traversal order.
    pub groups: Vec<CudaMultiQueryGroup>,
    /// Number of CUDA launches after grouping.
...
pub struct CudaMultiQueryExecutionScratch {
    ...
    grouped_queries: Vec<((u64, u64), Vec<CudaMultiQuery>)>,
    free_query_buckets: Vec<Vec<CudaMultiQuery>>,
    seen_queries: FxHashSet<u32>,
}

impl CudaMultiQueryExecutionScratch {
    ...
    pub fn try_with_capacity(query_count: usize) -> Result<Self, CudaMultiQueryExecutionError> {
    ...
    fn try_reserve_query_shape(
        &mut self,
        query_count: usize,
    ) -> Result<(), CudaMultiQueryExecutionError> {
        reserve_map(
    ...
    fn clear(&mut self) -> Result<(), CudaMultiQueryExecutionError> {
        ...
        let retained_bucket_count = self
            ...
            .ok_or(CudaMultiQueryExecutionError::ByteCountOverflow {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum CudaMultiQueryExecutionError {
    /// Duplicate query id.
    DuplicateQuery {
        /// Duplicate query id.
        query: u32,
...
impl std::fmt::Display for CudaMultiQueryExecutionError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::DuplicateQuery { query } => write!(
                f,
                "CUDA multi-query execution received duplicate query id {query}. Fix: assign unique ids before batch planning."
            ),
            Self::ZeroGraphHash { query } => write!(
                f,
                "CUDA multi-query {query} has graph_layout_hash=0. Fix: normalize and hash the resident graph before query batching."
            ),
            Self::ZeroTraversalKey { query } => write!(
                f,
                "CUDA multi-query {query} has traversal_key=0. Fix: emit a concrete traversal compatibility key before CUDA batching."
            ),
            Self::ZeroGraphUploadBytes { query } => write!(
                f,
                "CUDA multi-query {query} has graph_upload_bytes=0. Fix: pass the concrete resident graph topology byte count before CUDA batching."
            ),
            Self::GraphUploadBytesMismatch {
                ...
                f,
                "CUDA multi-query graph hash {graph_layout_hash} reported conflicting resident byte widths: expected {expected_bytes}, query {query} reported {actual_bytes}. Fix: canonicalize graph layout hashing and byte accounting before CUDA batching."
            ),
            Self::ZeroBudget => write!(
                f,
                "CUDA multi-query execution received a zero device budget. Fix: pass an explicit resident memory budget before planning."
            ),
            Self::ByteCountOverflow { field } => write!(
                f,
                "CUDA multi-query execution overflowed while computing {field}. Fix: shard the query batch before planning."
            ),
            Self::OverBudget {
                graph_layout_hash,
                traversal_key,
                ...
                f,
                "CUDA multi-query group graph={graph_layout_hash} traversal={traversal_key} requires {required_bytes} bytes but budget allows {budget_bytes}. Fix: split the group or raise the explicit CUDA budget."
            ),
            Self::StorageReserveFailed {
                ...
                f,
                "CUDA multi-query execution could not reserve {requested} {field} entries: {message}. Fix: shard the query batch before planning."
            ),
            Self::InternalInvariant { message } => write!(
                f,
                "CUDA multi-query execution violated an internal planner invariant: {message}. Fix: keep group counting and bucket indexing in one validated planning pass."
            ),
        }
    }
}

impl std::error::Error for CudaMultiQueryExecutionError {}
...
pub fn plan_cuda_multi_query_execution(
    ...
    let mut scratch = CudaMultiQueryExecutionScratch::try_with_capacity(queries.len())?;
    plan_cuda_multi_query_execution_with_scratch(queries, budget_bytes, &mut scratch)
}
...
pub fn plan_cuda_multi_query_execution_with_scratch(
    ...
    if budget_bytes == 0 {
        return Err(CudaMultiQueryExecutionError::ZeroBudget);
    }
    if queries.is_empty() {
        return Ok(CudaMultiQueryExecutionPlan {
            launch_count: 0,
            groups: Vec::new(),
            avoided_launches: 0,
    ...
    if queries.len() == 1 {
        ...
        if query.graph_layout_hash == 0 {
            return Err(CudaMultiQueryExecutionError::ZeroGraphHash { query: query.query });
        }
        if query.traversal_key == 0 {
            return Err(CudaMultiQueryExecutionError::ZeroTraversalKey { query: query.query });
        }
        if query.graph_upload_bytes == 0 {
            return Err(CudaMultiQueryExecutionError::ZeroGraphUploadBytes { query: query.query });
        }
        ...
        if resident_bytes > budget_bytes {
            return Err(CudaMultiQueryExecutionError::OverBudget {
        ...
        groups.push(CudaMultiQueryGroup {
            graph_layout_hash: query.graph_layout_hash,
            traversal_key: query.traversal_key,
            queries: query_ids,
    ...
    for query in queries {
        if !scratch.seen_queries.insert(query.query) {
            return Err(CudaMultiQueryExecutionError::DuplicateQuery { query: query.query });
        }
        if query.graph_layout_hash == 0 {
            return Err(CudaMultiQueryExecutionError::ZeroGraphHash { query: query.query });
        }
            ...
            .copied()
        {
            Some(expected_bytes) if expected_bytes != query.graph_upload_bytes => {
                return Err(CudaMultiQueryExecutionError::GraphUploadBytesMismatch {
                    graph_layout_hash: query.graph_layout_hash,
                    expected_bytes,
    ...
    for (&key, &query_count) in &scratch.group_query_counts {
        ...
        let mut queries = scratch.free_query_buckets.pop().unwrap_or_default();
        queries.clear();
        reserve_vec(
            &mut queries,
            query_count,
    ...
    for query in queries {
        ...
        let index = scratch.group_indices.get(&key).copied().ok_or(
            CudaMultiQueryExecutionError::InternalInvariant {
    ...
    for ((graph_layout_hash, traversal_key), group_queries) in &mut scratch.grouped_queries {
        group_queries.sort_unstable_by_key(|query| query.query);
        ...
        for group in &groups[first_new_group..] {
            ...
            graph_reuse = graph_reuse.checked_add(group.graph_reuse).map_err(|_| {
                CudaMultiQueryExecutionError::ByteCountOverflow {
    ...
    let launch_count = u32::try_from(groups.len()).map_err(|_| {
        CudaMultiQueryExecutionError::ByteCountOverflow {
            field: "launch count",
        }
    })?;

    Ok(CudaMultiQueryExecutionPlan {
        launch_count,
        groups,
        avoided_launches,
        avoided_host_fences,
...
fn append_memory_fit_groups(
    ...
    groups: &mut Vec<CudaMultiQueryGroup>,
) -> Result<(), CudaMultiQueryExecutionError> {
    ...
    while start < queries.len() {
        ...
        let mut query_ids = reserved_vec(queries.len() - start, "multi-query chunk query ids")?;
        ...
        let avoided = u32::try_from(query_ids.len() - 1).map_err(|_| {
            CudaMultiQueryExecutionError::ByteCountOverflow {
...
fn group_resident_bytes(
    ...
    output_bytes: u64,
) -> Result<u64, CudaMultiQueryExecutionError> {
...
fn checked_add(
    ...
    lhs.checked_add(rhs)
        .ok_or(CudaMultiQueryExecutionError::ByteCountOverflow { field })
}
...
fn checked_add_u32(
    ...
    rhs: u32,
    field: &'static str,
) -> Result<u32, CudaMultiQueryExecutionError> {
    lhs.checked_add(rhs)
        .ok_or(CudaMultiQueryExecutionError::ByteCountOverflow { field })
}
...
fn reserve_vec<T>(
    ...
    vec.try_reserve_exact(additional).map_err(|error| {
        CudaMultiQueryExecutionError::StorageReserveFailed {
            field,
            requested: capacity,
...
fn reserved_vec<T>(
    capacity: usize,
    field: &'static str,
) -> Result<Vec<T>, CudaMultiQueryExecutionError> {
    let mut vec = Vec::new();
...
where
    K: Eq + std::hash::Hash,
{
    let additional = if map.capacity() >= capacity {
        0
    } else {
...
where
    T: Eq + std::hash::Hash,
{
    let additional = if set.capacity() >= capacity {
        0
    } else {
...
mod tests {
    ...
    fn multi_query_batches_compatible_queries_over_one_resident_graph() {
        ...
        .expect("compatible queries should batch");
    ...
    fn multi_query_splits_compatible_group_to_fit_cuda_budget_without_reuploading_graph() {
        ...
        .expect("compatible CUDA queries should split into budget-fit resident chunks");
    ...
    fn multi_query_later_chunks_still_count_resident_graph_memory() {
        assert_eq!(
            plan_cuda_multi_query_execution(
                &[
                    query(1, 0xabc, 0x10, 100, 100, 10, 10),
                    query(2, 0xabc, 0x10, 100, 100, 10, 10),
                ],
            ...
            CudaMultiQueryExecutionError::OverBudget {
    ...
    fn multi_query_splits_incompatible_graph_or_traversal_keys() {
        ...
        .expect("incompatible queries should become separate groups");
    ...
    fn multi_query_grouping_avoids_tree_lookup_per_query() {
        ...
        assert!(
            ...
            "Fix: CUDA multi-query grouping should hash query ids and group indices, then sort final groups once for deterministic output."
        );
    }

    #[test]
    fn multi_query_planner_reuses_caller_owned_grouping_scratch() {
        let mut scratch = CudaMultiQueryExecutionScratch::try_with_capacity(128)
            .expect("multi-query scratch should reserve");
        let wide = (0..128)
            .map(|index| query(index, 0xabc, 0x10, 4_096, 4, 8, 4))
            .collect::<Vec<_>>();
        let first = plan_cuda_multi_query_execution_with_scratch(&wide, 16_384, &mut scratch)
            .expect("wide compatible query batch should plan");
        ...
        assert!(
            ...
            "Fix: CUDA multi-query scratch must retain inner grouped-query bucket capacity across planning calls"
        );

        let second = plan_cuda_multi_query_execution_with_scratch(
            &[
        ...
        .expect("smaller incompatible query batch should reuse previous scratch");
        ...
        let src = include_str!("multi_query_execution.rs");
        assert!(
            src.contains("pub fn plan_cuda_multi_query_execution_with_scratch"),
            "Fix: release callers need a scratch-aware CUDA multi-query planning path"
        );
        assert!(
            src.contains("scratch.grouped_queries.sort_unstable_by_key"),
            "Fix: deterministic CUDA multi-query output should sort retained scratch buckets in place"
        );
    }
    ...
    fn multi_query_planner_staging_reserves_fallibly() {
        ...
        assert!(
            src.contains("CudaMultiQueryExecutionScratch::try_with_capacity(queries.len())?")
                && src.contains("scratch.try_reserve_query_shape(queries.len())?")
                && src.contains("fn reserve_vec<T>(")
                && src.contains("fn reserve_map<K, V>(")
                && src.contains("fn reserve_set<T>(")
                && src.contains("StorageReserveFailed"),
            "Fix: CUDA multi-query execution planning must reserve scratch and output staging fallibly."
        );
        assert!(
            !src.contains(concat!("FxHashMap::with_capacity", "_and_hasher"))
                && !src.contains(concat!("FxHashSet::with_capacity", "_and_hasher"))
                && !src.contains(concat!("Vec::with_capacity", "(query_count)"))
                && !src.contains(concat!(
                    "Vec::with_capacity",
                    "(scratch.grouped_queries.len())"
                ))
                && !src.contains(concat!("Vec::with_capacity", "(queries.len() - start)"))
                && !src.contains(concat!("groups: vec![", "CudaMultiQueryGroup"))
                && !src.contains(concat!("queries: vec![", "query.query]"))
                && !src.contains(concat!("scratch.group_indices", ".reserve(queries.len())"))
                && !src.contains(concat!(
                    "scratch.grouped_queries",
                    ".reserve(queries.len())"
                ))
                && !src.contains(concat!("scratch.seen_queries", ".reserve(queries.len())")),
            "Fix: CUDA multi-query release planning must not use infallible staging allocation."
        );
    }
    ...
    fn multi_query_rejects_invalid_inputs_and_budget_overflow() {
        assert_eq!(
            plan_cuda_multi_query_execution(&[query(1, 0, 1, 8, 1, 1, 1)], 128)
                .expect_err("missing graph hash should fail"),
            CudaMultiQueryExecutionError::ZeroGraphHash { query: 1 }
        );
        assert_eq!(
            plan_cuda_multi_query_execution(&[query(1, 1, 1, 0, 1, 1, 1)], 128)
                .expect_err("zero graph bytes should fail"),
            CudaMultiQueryExecutionError::ZeroGraphUploadBytes { query: 1 }
        );
        assert_eq!(
            plan_cuda_multi_query_execution(
            ...
            CudaMultiQueryExecutionError::GraphUploadBytesMismatch {
                graph_layout_hash: 1,
                expected_bytes: 8,
                actual_bytes: 16,
                query: 2,
        ...
        assert_eq!(
            ...
            CudaMultiQueryExecutionError::DuplicateQuery { query: 1 }
        );
        assert_eq!(
            plan_cuda_multi_query_execution(&[query(2, 1, 1, 128, 16, 16, 16)], 127)
