...
payloads for the supported compile-only surface. Those payloads embed the
compiled vyre program as a `VYRECOB2` v3 section. It is the library behind
the `vyrec` binary at `tools/vyrec/`.
...
Release parser evidence must prove more than "some C file compiled".
The frontend contract covers C syntax recognition, AST fidelity,
actionable diagnostics, byte-span/location preservation, preprocessor
behavior for includes and macros, GNU extension handling, and honest
unsupported-feature diagnostics. Full C compiler lowering is outside
the `0.4.1` parser milestone; silent acceptance of unsupported syntax
is not.
