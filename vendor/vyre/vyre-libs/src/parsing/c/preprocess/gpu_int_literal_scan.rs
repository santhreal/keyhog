...
/// Maximum suffix bytes consumed (`u`/`U`/`l`/`L`).
pub const MAX_SUFFIX: u32 = 4;

/// Build the 17b.2 integer-literal scanner `Program`.
///
/// `source_len` declares the source buffer's byte length so the IR
/// validator knows the bound on the `safe_load` clamps.
#[must_use]
    ...
    let body: Vec<Node> = vec![
        Node::if_then(
            ...
            vec![
                ...
                // Otherwise consume up to 4 trailing u/U/l/L suffix bytes.
                    ...
                    vec![Node::loop_for(
                        ...
                        vec![Node::if_then(
                            ...
                            vec![
                                ...
                                Node::if_then_else(
                                    Expr::eq(Expr::var("is_suffix"), Expr::u32(1)),
                                    vec![Node::assign(
                                        "idx",
                                        Expr::add(Expr::var("idx"), Expr::u32(1)),
                                    )],
