...
- 2026-06-11 · KH-L-0002 (partial — version-matrix GREEN) · vyre 66481599eb + afd3e00518 · architecture · Drove vyre's OWN release machinery to first-gate-green at 0.6.1: (1) hoisted `required_*`/`package_verify_passed` above `[tags]` in release-train.toml (TOML was binding them into [tags], so the ReleaseTrainData deserializer saw them missing and `version-matrix` couldn't parse at all); (2) synced `vyre-grammar-gen` 0.6.0→0.6.1 (folded-in crate left at old version) + relocked. `xtask version-matrix` now exits 0, blockers `[]`. The release tooling parses + runs end-to-end; workspace is version-consistent at 0.6.1. NEXT: the 0.6.2 lockstep bump (28 vyre crates + workspace.deps + release-train tokens/tags, weir/vyrec untouched) → re-run version-matrix → then the heavier release gates (`release-gate`/`release-evidence`/conformance+signed certs) → keyhog override `=0.6.1`→`=0.6.2` + prove both trees build → KH-L-0006 publish (gated, irreversible).
