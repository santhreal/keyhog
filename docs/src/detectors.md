...
*   **`entropy_fallback`** (table, optional): Identity metadata for synthetic entropy findings owned by this detector. `id` must use the `entropy-` namespace, `name`/`service` must be non-empty, and `service` must be `generic`. Both regex-kind and phase-2 generic owners may declare it when they own an entropy class. Embedded generic detectors declare this block; custom legacy specs may omit it and use the compatibility identity.
