...
|---|---|---|
| Publish a release | `scripts/release.py "$NEXT_VERSION" --publish` | Builds one candidate and refreshes every committed benchmark surface before version preparation. |
...
The release path is the only command that orders benchmark refresh, generated
commit ownership, version preparation, gates, signing, and publication.
...
pushed, or the exact remote signed annotated tag. It verifies the existing tag's
commit, OpenPGP signature, and configured primary-key fingerprint. It never
moves or replaces a release tag.
