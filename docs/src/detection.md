...
| pattern `required_literals` | Routes the owning regex only after at least one AST-proven necessary ASCII literal is present; useful for distinctive three-byte infixes that the global inner-literal floor intentionally excludes | Omitting it leaves the regex in its ordinary prefix, keyword, or always-active route; an unsound declaration rejects the detector |
...
- **Synthetic entropy identity:** every active entropy owner declares
  ...
  compiled scanner uses the complete metadata from the active detector corpus
