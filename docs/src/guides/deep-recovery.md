...
| Every reachable blob | `keyhog scan --git-blobs .` | Deduplicated blobs from every ref, including branches you have not checked out and refs that only exist in `packed-refs`, plus annotated tag messages, stashes, notes, and dangling objects that a rewrite or an amend left behind. Enumeration diffs each commit against its parents and keeps added, changed, and deleted blob sides, so a credential that was committed and later removed stays visible without rewalking every historical tree. Every named ref tip plus HEAD is still fully walked once so `--max-commits` cannot hide untouched blobs on non-newest branches or detached CI checkouts. | Blobs that decode as binary, including committed archives. |
...
Measured on release-fast v0.5.68, one Base64-wrapped AWS key pair at the end of
