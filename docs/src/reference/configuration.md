...
| Entropy threshold | **4.5** | `[scan].entropy_threshold` | `--entropy-threshold` | Scan-wide Shannon-entropy control in bits/byte. It is not a blanket replacement for detector `entropy_low`/`entropy_high`/`entropy_very_high`/length-bucket floors: each detection path composes it with the owning detector's evidence band. For keyword-free admission the exact formula and margin are owned by that detector's TOML and shown by `keyhog explain`. The byte-entropy domain is `[0.0, 8.0]`; non-finite and out-of-range requests fail closed. |
