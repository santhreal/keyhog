...
| Scanner tuning | compiled scanner defaults | `[tuning]` | - | Detection/recall route gates that affect engine work selection. Configuration identity and runtime snapshots use the same complete resolved record. Ambient `KEYHOG_*` tuning env vars are ignored. |
