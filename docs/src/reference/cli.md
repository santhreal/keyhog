...
| `--matcher-cache` | `DIR\|off` | platform cache dir | Override the MatcherArtifact cache directory, or `off` to disable. Persists the eager compiled matcher graph across invocations. Distinct from `--cache-dir` Hyperscan `.db` shards. Config: `[system].matcher_cache`; this flag overrides it. |
...
| `--matcher-cache` | `DIR\|off` |  | Override the MatcherArtifact cache directory, or `off` to disable |
| `-d`, `--detectors` | `DETECTORS` | `detectors` | Detector TOML directory. When omitted, KeyHog discovers an installed corpus or uses the embedded corpus. An explicitly named missing path is an error |
