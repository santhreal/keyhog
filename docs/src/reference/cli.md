...
| `--incremental` |  |  | Incremental scan: skip files whose content hash matches the cached `~/.cache/keyhog/merkle.idx`. After the scan completes, the index is updated with the current file contents. If acquisition yields only unchanged files, backend routing and scanner dispatch do not start. Pass `--incremental-cache <path>` to override the location |
