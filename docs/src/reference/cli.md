...
| `--incremental` |  |  | Skip filesystem entries that match the spec-bound Merkle index. The index is updated after a successful in-process scan or a `--daemon=mass` daemon-local filesystem scan. If acquisition yields only unchanged files, backend routing and scanner dispatch do not start |
