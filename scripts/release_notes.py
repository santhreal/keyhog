...
_CHANGE_CATEGORY_RE = re.compile(
...
def validate_release_notes(notes: str, tag: str) -> str:
    """Require categorized, substantive notes instead of a changelog pointer."""

    rendered = notes.strip()
    if not rendered:
        raise ReleaseNotesError(f"{tag} changelog section is empty")
    prose_lines = [
        line for _index, line in _lines_outside_fences(rendered.splitlines())
    ]
    if "see changelog" in "\n".join(prose_lines).casefold():
        raise ReleaseNotesError(
            f"{tag} release notes contain a placeholder changelog pointer"
        )
    if not any(_CHANGE_CATEGORY_RE.fullmatch(line) for line in prose_lines):
        raise ReleaseNotesError(
            f"{tag} release notes need an Added, Changed, Deprecated, Removed, Fixed, or Security category"
        )
    if not any(line.startswith("- ") and line[2:].strip() for line in prose_lines):
        raise ReleaseNotesError(
            f"{tag} release notes need at least one concrete change"
        )
    return rendered + "\n"
...
def extract_release_notes(changelog: Path, tag: str) -> str:
    ...
    if len(matches) != 1:
        raise ReleaseNotesError(
            f"CHANGELOG.md must contain exactly one release heading for {tag}; found {len(matches)}"
        )
    start = matches[0][0] + 1
    end = next(
