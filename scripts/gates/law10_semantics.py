...
CATEGORIES: dict[str, re.Pattern[str]] = {
    "no_runtime_effect": re.compile(
        ...
        r"recover the inner guard|data still valid|never blocks|test-only|test helper|"
        r"consumer (?:is )?(?:already )?closed|consumer closed|no recipient remains|"
        r"receiver is retained|receiver .* retained|cannot fail (?:during|while)|"
        r"joining only reclaims|poison recovery retains?|poisoned .* recovery retains?|"
        r"recovers? the (?:retained|one-shot) .* (?:value|slot)|"
        r"same completion flag|same handle|channel closure is the iterator eof|"
        r"pending rows are drained|no further .* can consume)\b"
    ),
    "reporting_only": re.compile(
        ...
        r"health/diagnostic|error-message string|output|result metadata|"
        r"(?:debug|profiler|profiling) (?:duration|accounting))\b"
    ),
    "recall_preserving": re.compile(
        ...
        r"preserves deterministic|preserved|primary path|scan reparses|"
        r"(?:scan|matcher|matching|decoder|line indexing) (?:behavior |semantics )?is unchanged|"
        r"identical (?:scan semantics|matching behavior)|"
        r"(?:scan )?findings? (?:(?:remain|are) )?(?:unmodified|untouched)|"
        r"cannot suppress (?:a )?finding|no .* (?:is )?dropped|no claim is lost|"
        r"never a finding path|not a finding or coverage path|"
        r"keeps? .*?(?:matching|work|decode generation) enabled|"
        r"(?:runs?|performs?) (?:the )?full|runs? .* in full|runs normalization|"
        r"recomputes? the full|rebuilds? context|new exact payload class|"
        r"uses? the exact original line index|still leaf-scan|still scans?|"
        r"(?:sourceerror|original row) remains|every original row remains|"
        r"preserves every valid ascii secret byte|cannot alter production scan results|"
        r"no production result is suppressed|no effect on scan findings|"
        r"cannot truncate .* further|cannot create a stronger target|"
        r"cannot suppress findings|cannot bypass retention|"
        r"source errors are unchanged|checks still run|"
        r"fetch results|result remains authoritative|retain every target)\b"
    ),
    "fail_closed_or_security_safe": re.compile(
        ...
        r"dns-resolved|re-screened|aborts to `?none`|field-parse aborts|"
        r"reaches .*ok_or_else|propagated by .*ok_or_else|remains an error|"
        r"rejects the (?:plan|artifact)|aborts? .*accounting|"
        r"(?:explicit )?sourceerror returned|returned as an explicit source error|"
        r"(?:errors?|failures?) remain explicit|original build error|"
        r"cannot hide .* error|returns? .* (?:error|failures)|"
        r"(?:sourceerror|fetch errors) (?:still )?propagate|"
        r"propagates every sourceerror)\b"
    ),
    "loud_counted_or_surfaced": re.compile(
        r"(?i)\b(?:loud|surface|surfaced|visible|operator must see|warn|warned|warning|"
        ...
        r"emitted|reads as full .* coverage|coverage reflects|unavailable provenance|"
        r"explicit sourceerror|source error|coverage gap|record the uncovered region|"
        r"worker panic .* surfaces|worker join .* surfaces .*sourceerror)\b"
    ),
    "intentional_default": re.compile(
        ...
        r"caller defaults|default prefix|optional|unset|absent|absence means|"
        r"empty/absent|missing/non-string|path-less|none is valid|"
        r"anonymous s3|infer from url|base64 zero-pad|hashes as none|"
        r"distinct cache-key state|empty names|source_\{i\}|canonical .* minimum|"
        r"same canonical digest|exact effective cap|maximum confidence decay|"
        r"separate control pool|multiplier bound|representable origin|"
        r"spool-buffer sizing)\b"
    ),
}
