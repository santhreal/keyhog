<p align="center">
  <img src="docs/assets/keyhog-banner.svg" alt="keyhog - secret scanner - 908 detectors - gpu" width="560" />
</p>
...
repository collections, S3/GCS/Azure Blob buckets, and running systems for leaked credentials. **908 service-specific detectors**,
...
no libstdc++ link. Same 908 detectors, same ML/entropy/decode/multiline
...
keyhog compiles its 908 detectors into a shared trigger/extraction plan,
...
⚡ 16 cores | SIMD: AVX-512 | Hyperscan | 908 detectors (6054 patterns) | backend=simd-regex
```
...
<p align="center">
  <img src="demo/keyhog-doctor.gif" alt="keyhog doctor — host probe (RTX 5090, AVX-512, Hyperscan), one keyhog on PATH, 908 embedded detectors, and a four-way self-test (scan engine, GPU scan path, GPU literal set, GPU MoE shader vs CPU reference) all reporting PASS, then 'keyhog is healthy'" width="860" />
</p>
...
908 service-specific detectors with checksum / companion validation:
...
loads all 908 with severity + service + keyword filter.
...
detectors/    908 TOML files (data, not code)
