...
printf 'TOKEN=%s\n' "$secret_one" >"$tmp/visible.env"
printf 'TOKEN=%s\n' "$hidden_secret" >"$tmp/hidden.env"
...
# an empty report.
set +e
"$bin" "${common[@]}" --format json "$tmp" >"$tmp/exclude.json" 2>"$tmp/exclude.err"
rc=$?
set -e
[[ $rc -eq 1 ]] || { cat "$tmp/exclude.err" >&2; echo "exclude scan exited $rc, expected 1" >&2; exit 1; }
assert_report \
  'length == 1 and .[0].detector_id == "stripe-secret-key" and (.[0].location.file_path | endswith("visible.env"))' \
  "$tmp/exclude.json" "exclude scan did not preserve exactly the visible Stripe finding"
assert_redacted "$secret_one" "$tmp/exclude.json"
assert_redacted "$hidden_secret" "$tmp/exclude.json"
...
"$bin" "${common[@]}" --create-baseline "$tmp/baseline.json" --format json \
  "$tmp/visible.env" >"$tmp/create-baseline.json" 2>"$tmp/create-baseline.err"
set +e
"$bin" "${common[@]}" --baseline "$tmp/baseline.json" --format json \
  "$tmp/visible.env" >"$tmp/known.json" 2>"$tmp/known.err"
rc=$?
set -e
[[ $rc -eq 0 ]] || { cat "$tmp/known.err" >&2; echo "baseline rescan exited $rc, expected 0" >&2; exit 1; }
assert_report 'type == "array" and length == 0' "$tmp/known.json" \
  "baseline rescan did not suppress the reviewed finding"

printf 'TOKEN=%s\n' "$secret_two" >"$tmp/new.env"
set +e
"$bin" "${common[@]}" --baseline "$tmp/baseline.json" --format json \
  "$tmp" >"$tmp/new-only.json" 2>"$tmp/new-only.err"
rc=$?
set -e
[[ $rc -eq 1 ]] || { cat "$tmp/new-only.err" >&2; echo "new-finding scan exited $rc, expected 1" >&2; exit 1; }
assert_report \
  'length == 1 and .[0].detector_id == "stripe-secret-key" and (.[0].location.file_path | endswith("new.env"))' \
  "$tmp/new-only.json" "baseline scan did not isolate exactly the new Stripe finding"
assert_redacted "$secret_two" "$tmp/new-only.json"
...
set +e
"$bin" "${common[@]}" --format jsonl "$tmp/visible.env" \
  >"$tmp/finding.jsonl" 2>"$tmp/finding-jsonl.err"
rc=$?
set -e
[[ $rc -eq 1 ]] || { cat "$tmp/finding-jsonl.err" >&2; echo "JSONL scan exited $rc, expected 1" >&2; exit 1; }
if ! jq -se 'length == 1 and .[0].detector_id == "stripe-secret-key"' \
  "$tmp/finding.jsonl" >/dev/null; then
  echo "JSONL reporter did not emit exactly one canonical Stripe finding" >&2
  jq -s . "$tmp/finding.jsonl" >&2 || true
  exit 1
fi
assert_redacted "$secret_one" "$tmp/finding.jsonl"

# Stdin uses the same detector and exit contract without filesystem metadata.
set +e
printf 'TOKEN=%s\n' "$secret_one" \
  | "$bin" "${common[@]}" --stdin --format json >"$tmp/stdin.json" 2>"$tmp/stdin.err"
rc=$?
set -e
[[ $rc -eq 1 ]] || { cat "$tmp/stdin.err" >&2; echo "stdin scan exited $rc, expected 1" >&2; exit 1; }
assert_report 'length == 1 and .[0].detector_id == "stripe-secret-key"' \
  "$tmp/stdin.json" "stdin reporter did not emit exactly one canonical Stripe finding"
assert_redacted "$secret_one" "$tmp/stdin.json"
...
printf 'payload=%s\n' "$encoded" >"$tmp/encoded.txt"
set +e
"$bin" "${common[@]}" --format json "$tmp/encoded.txt" \
  >"$tmp/decoded.json" 2>"$tmp/decoded.err"
rc=$?
set -e
[[ $rc -eq 1 ]] || { cat "$tmp/decoded.err" >&2; echo "decode-through scan exited $rc, expected 1" >&2; exit 1; }
assert_report 'length == 1 and .[0].detector_id == "stripe-secret-key"' \
  "$tmp/decoded.json" "decode-through did not emit exactly one canonical Stripe finding"
assert_redacted "$secret_one" "$tmp/decoded.json"
...
set +e
"$bin" "${common[@]}" --precision --format sarif "$tmp/visible.env" \
  >"$tmp/precision.sarif" 2>"$tmp/precision.err"
rc=$?
set -e
[[ $rc -eq 1 ]] || { cat "$tmp/precision.err" >&2; echo "precision SARIF scan exited $rc, expected 1" >&2; exit 1; }
assert_report \
  '.version == "2.1.0" and ([.runs[].results[]?] | length == 1) and .runs[0].results[0].ruleId == "stripe-secret-key"' \
  "$tmp/precision.sarif" "precision SARIF did not emit exactly one canonical Stripe finding"
assert_redacted "$secret_one" "$tmp/precision.sarif"
