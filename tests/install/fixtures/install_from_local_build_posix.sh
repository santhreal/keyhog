...
#
# Usage:  install_from_local_build_posix.sh [path-to-keyhog-binary]
...
run_install() {  # args... -> install.sh; output captured to $OUT, status to $RC
    OUT=$(sh "$INSTALL_SH" --from-file="$ARTIFACT" --install-dir="$PREFIX" --no-color "$@" 2>&1)
    RC=$?
}
...
{ [ "$RC" != "0" ] && printf '%s' "$OUT" | grep -q "SHA256 mismatch"; } \
    && ok_ "B.2 tampered .sha256 is refused before overwrite" \
    || bad_ "B.2 tampered .sha256 is refused before overwrite" "rc=$RC"

rm -f "$STAGE/keyhog.sha256" "$KEYHOG"
OUT=$(sh "$INSTALL_SH" --from-file="$STAGE/keyhog" --install-dir="$PREFIX" --no-color --yes --no-prompt 2>&1); RC=$?
{ [ "$RC" != "0" ] && printf '%s' "$OUT" | grep -q "Refusing to install an unverified"; } \
