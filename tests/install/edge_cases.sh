...
rm -rf "$h"
# 6.8 no .sha256 published: skip verification, still installs
h=$(newhome)
out=$(KEYHOG_VERSION=v9.9.9 MOCK_ASSET="$FIX_DIR/fake_keyhog_healthy" MOCK_SHA=absent run_install "$sb" "$h" -- --no-prompt); st=$?
expect_match  "6.8 absent .sha256 -> skip note" "skipping checksum verification" "$out"
expect_status "6.9 absent .sha256 still installs" 0 "$st"
expect_exec   "6.10 binary present despite no checksum" "$h/.local/bin/keyhog"
rm -rf "$h"
# 6.11 install dir created when missing
...
expect_file "6.11 mkdir -p created install dir" "$h/.local/bin/keyhog"
rm -rf "$h"
rm -rf "$sb"

# ======================================================================
# 7. CUDA -> CPU fallback on missing cuda asset
# ======================================================================
printf '\n[7] cuda asset 404 -> cpu fallback\n'
reset_mocks
sb=$(build_sandbox Linux x86_64 yes yes yes); h=$(newhome)
...
expect_match  "7.1 falls back to cpu asset" "Falling back to keyhog-linux-x86_64" "$out"
expect_status "7.2 fallback install exits 0" 0 "$st"
expect_exec   "7.3 binary installed via fallback" "$h/.local/bin/keyhog"
rm -rf "$h"; h=$(newhome)
out=$(KEYHOG_VERSION=v9.9.9 KEYHOG_VARIANT=cuda MOCK_ASSET=404 MOCK_FALLBACK=404 run_install "$sb" "$h" -- --no-prompt); st=$?
expect_match  "7.4 both 404 errors" "Neither .* could be downloaded|could not be downloaded" "$out"
expect_status "7.5 both 404 exits 1" 1 "$st"
rm -rf "$sb" "$h"
...
# 12.3 no sha tool at all -> warn + skip, still installs
sb=$(build_sandbox Linux x86_64 no no no); rm -f "$sb/bin/sha256sum" "$sb/bin/shasum"; h=$(newhome)
out=$(KEYHOG_VERSION=v9.9.9 MOCK_ASSET="$FIX_DIR/fake_keyhog_healthy" MOCK_SHA=match run_install "$sb" "$h" -- --no-prompt); st=$?
expect_match  "12.3 no sha tool warns" "no sha256sum / shasum" "$out"
expect_status "12.4 no sha tool still installs" 0 "$st"
expect_exec   "12.5 binary present without checksum tool" "$h/.local/bin/keyhog"
rm -rf "$sb" "$h"
